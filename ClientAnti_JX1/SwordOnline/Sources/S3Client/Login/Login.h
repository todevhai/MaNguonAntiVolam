...
public:
	...
	int SelectDelRole(int nIndex, const KSG_PASSWORD &crSupperPassword);
	//请求新建游戏角色
	int	CreateRole(KRoleChiefInfo* pCreateInfo);
	//请求删除游戏角色
	int	DeleteRole(tagNotifyPlayerLogin* pResponse, const KSG_PASSWORD &crSupperPassword);
	//通知等待返回结果超时了
	void NotifyTimeout();
	//通知要开始游戏了
...
private:
	...
	void	ProcessGetRoleResponse(tagNotifyPlayerLogin* pResponse);	//删除角色列表的响应
