...
{
	0x0a001e13,		//IIEP_NORMAL ??/??/?? 0,30,19
	0x0a2c0000,		//IIEP_NOT_USEABLE ???/???? 44,0,0
	0x0a654915,		//IIEP_SPECIAL ??????? 101,73,21
...
//	m_data=NULL;
//	m_lasttimeframe=0;
//	m_col[0]=minframeitemback;
//	m_typeframe=toframeitemback;
//	m_check=false;
...
/////
int KWndObjectBox::GetRectItem(int _w, int _h)
{
	int i;
	if	   (_w == 1 && _h == 1) i = 0;
...
void KWndObjectBox::DrawPoint(int x, int y, unsigned int wColor)
{
	KRUPoint Rect;
...
void KWndObjectBox::DrawLight(int vongmau,int x, int y, int _w, int _h, int unit, int unitboder)
{	
	int r_color = vongmau; // 1::Yellow, 2::Purple, 3::White 	
	x = x - 1;
	y = y - 1;
	int w = unit + 1;
	int h = unitboder + 1;
...
void KWndObjectBox::PaintWindow()
{
	...
	if (m_Object.uGenre != CGOG_NOTHING && g_pRepresentShell)
	{
		...
		if ((m_Object.uGenre == CGOG_ITEM/* || m_Object.uGenre == CGOG_PLAYERSELLITEM*/) && (g_pCoreShell->GetGenreItem(m_Object.uId) == 0))
		{
			//int ItemType = g_pCoreShell->GetTypeItem(m_Object.uId);
			//if (ItemType == 1)
			//{
	//		m_Object11.Run(m_nAbsoluteLeft,m_nAbsoluteTop,m_Width,m_Height,ehoangkim,m_Width);
			//}
			//doan nay ong tim item nao la hoang kim =ham tren ,server cua toi chua co ham
			//tim item hoang kim
		
			//draw_vongsang(m_Object.m_posvs,m_Object.m_dirvs,m_Object.m_vs_lastframe,
				//m_nAbsoluteLeft,m_nAbsoluteTop,m_Width,m_Height,m_Width+m_Height/2);
			if (g_pCoreShell->GetTypeItem(m_Object.uId) == 1)
			{
				DrawBorder(m_bCountFrame, m_nFrame, m_ulTimeDelay, m_nAbsoluteLeft, m_nAbsoluteTop, m_Width, m_Height, GoldItem);
				//this->DrawLight(1,m_nAbsoluteLeft,m_nAbsoluteTop, m_Object.DataW, m_Object.DataH, m_Width, m_Height);
				draw_vongsang(m_Object.m_posvs,m_Object.m_dirvs,m_Object.m_vs_lastframe,
					m_nAbsoluteLeft-1,m_nAbsoluteTop-1,m_Width+1,m_Height+1,((m_Width-12)+(m_Height-12))/2,ehoangkim);
			}
			else if (g_pCoreShell->GetTypeItem(m_Object.uId) == 2)
			{
				DrawBorder(m_bCountFrame, m_nFrame, m_ulTimeDelay, m_nAbsoluteLeft, m_nAbsoluteTop, m_Width, m_Height, GoldItem);
				//this->DrawLight(2,m_nAbsoluteLeft,m_nAbsoluteTop, m_Object.DataW, m_Object.DataH, m_Width, m_Height);
				draw_vongsang(m_Object.m_posvs,m_Object.m_dirvs,m_Object.m_vs_lastframe,
					m_nAbsoluteLeft-1,m_nAbsoluteTop-1,m_Width+1,m_Height+1,((m_Width-12)+(m_Height-12))/2,ehuyenkim);

			}

		}
		
		//tao mang point for tu:0-n ve
...
int	KWndObjectBox::Init(KIniFile* pIniFile, const char* pSection)
{
	...
	//	m_Object.Init(m_nAbsoluteLeft,m_nAbsoluteTop,m_Width,m_Height,ehoangkim,m_Width);
		return true;
...
void KWndObjectMatrix::DrawLight(int vongmau, int x, int y, int _w, int _h, int unit, int unitboder)
{
	int r_color = vongmau; // 1::Yellow, 2::Purple, 3::White 
	x = x - 1;
	y = y - 1;
...
void KWndObjectMatrix::PaintWindow()
{
	...
	//	CVongSang *pvongsang=&m_vongsang[i];
		Shadow.Color.Color_dw = 0;
		...
		if ((pObj->uGenre == CGOG_ITEM /*|| pObj->uGenre == CGOG_PLAYERSELLITEM*/) && (g_pCoreShell->GetGenreItem(pObj->uId) == 0))
		{
			
			//int ItemType = g_pCoreShell->GetTypeItem(m_Object.uId);
			//if (ItemType == 1)
			//{
		//	pObj->Init(Shadow.oPosition.nX, Shadow.oPosition.nY, width, height,ehoangkim,m_Width);
		//	pObj->Run(Shadow.oPosition.nX, Shadow.oPosition.nY, width, height,ehoangkim,m_Width);
			//}
			if (g_pCoreShell->GetTypeItem(pObj->uId) == 1)
			{
				DrawBorder(m_bCountFrame, m_nFrame, m_ulTimeDelay, Shadow.oPosition.nX, Shadow.oPosition.nY, width, height, GoldItem);
				//this->DrawLight(1,Shadow.oPosition.nX,Shadow.oPosition.nY, pObj->DataW, pObj->DataH , m_nUnitWidth, m_nUnitBorder);
				draw_vongsang(pObj->m_posvs,pObj->m_dirvs,pObj->m_vs_lastframe,
					Shadow.oPosition.nX,Shadow.oPosition.nY,width,height,((width-12)+(height-12))/2,ehoangkim);
			}
			else if (g_pCoreShell->GetTypeItem(pObj->uId) == 2)
			{
				DrawBorder(m_bCountFrame, m_nFrame, m_ulTimeDelay, Shadow.oPosition.nX, Shadow.oPosition.nY, width, height, GoldItem);
				//this->DrawLight(2,Shadow.oPosition.nX,Shadow.oPosition.nY, pObj->DataW, pObj->DataH , m_nUnitWidth, m_nUnitBorder);
				draw_vongsang(pObj->m_posvs,pObj->m_dirvs,pObj->m_vs_lastframe,
					Shadow.oPosition.nX,Shadow.oPosition.nY,width,height,((width-12)+(height-12))/2,ehuyenkim);
...
{	//xem lai phah nay
	if (pObject && nCount > 0)
	{
		...
		//	CVongSang* pvongsang = (CVongSang*)realloc(m_vongsang, sizeof(CVongSang) * (m_nNumObjects + nValidCount));
			
			if (pNewList)
			{//	m_vongsang=pvongsang;
				m_pObjects = pNewList;
				for (i = 0; i < nCount; i++)
				{
					if (pObject[i].uGenre)
	...
	if (pObject)
	{
		...
		for (int i = 0; i < m_nNumObjects; i++)
		{
				...
				pHolded->DataY == pObject->DataY)
			{
				...
				//	m_vongsang[i]=m_vongsang[i+1];//chu y tim hieu ,neu no huy thi
					//bo nho ro ri tinh sao
				}
				return true;
			}
		}
