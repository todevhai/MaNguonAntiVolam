...
#define SCHEME_INI_ITEM	"HanhTrang.ini"

KUiItemEX* KUiItemEX::m_pSelf = NULL;


//--------------------------------------------------------------------------
//	功能：如果窗口正被显示，则返回实例指针
//--------------------------------------------------------------------------
KUiItemEX* KUiItemEX::GetIfVisible()
{
...
//--------------------------------------------------------------------------
KUiItemEX* KUiItemEX::OpenWindow()
{
	if (m_pSelf == NULL)
	{
		m_pSelf = new KUiItemEX;
		if (m_pSelf)
			m_pSelf->Initialize();
	}
	if (m_pSelf)
...
void KUiItemEX::Initialize()
{
	...
	m_ItemEX.SetContainerId((int)UOC_ITEM_EX);
	char Scheme[256];
...
void KUiItemEX::UpdateData()
{
	...
	int nCount = g_pCoreShell->GetGameData(GDI_ITEM_EX, 0, 0);
	if (nCount == 0)
		return;
...
// 功能	: 载入界面方案
// -------------------------------------------------------------------------
void KUiItemEX::LoadScheme(const char* pScheme)
{
	char		Buff[128];
	KIniFile	Ini;
...
void KUiItemEX::OnItemPickDrop(ITEM_PICKDROP_PLACE* pPickPos, ITEM_PICKDROP_PLACE* pDropPos)
{
