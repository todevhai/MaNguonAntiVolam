...
void KImmediaItem::PaintWindow()
{
	...
	if (g_pCoreShell)
	{
		...
		/* O giu chieu (nNum = 0) cung phai ve lai so, khong thi icon chieu che mat. */
		if ((nNum >= 1 || m_Object.uGenre != CGOG_NOTHING) && m_nIndex >= 0 && m_nIndex < 9)
		{
...
//	_ASSERT(pIni);
	...
	m_MainIcon    .Init(pIni, "MainIcon");
	m_TuongTacNhom    .Init(pIni, "TuongTacNhom");
	m_TuongTacNguoiChoi    .Init(pIni, "TuongTacNguoiChoi");
	m_Market    .Init(pIni, "Market");
	m_DichChuyen  .Init(pIni, "DichChuyen");
	m_TongKim  .Init(pIni, "TongKim");
...
void KUiPlayerBar::Initialize()
{
	...
	AddChild(&m_MainIcon);
	AddChild(&m_Market);
	AddChild(&m_DichChuyen);
	AddChild(&m_TongKim);
	AddChild(&m_TuongTacNguoiChoi);
	AddChild(&m_TuongTacNhom);
...
int KUiPlayerBar::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)
{
	...
	case WND_N_BUTTON_CLICK:
		...
		{
			if (KUiTongKimPhongVan::GetIfVisible())
				KUiTongKimPhongVan::CloseWindow(false);
			else
				KUiTongKimPhongVan::OpenWindow();				
		}
		else if (uParam == (unsigned int)(KWndWindow*)&m_TuongTacNhom)
			g_pCoreShell->OperationRequest(GOI_PLAYER_ACTION, HT_CN, 0);
		else if (uParam == (unsigned int)(KWndWindow*)&m_TuongTacNguoiChoi)
			g_pCoreShell->OperationRequest(GOI_PLAYER_ACTION, HT_CN, 0);
		else if (uParam == (unsigned int)(KWndWindow*)&m_ChannelOpenBtn)
		{
		}
...
void KUiPlayerBar::OnSend(BOOL bDirectSend)
{
	...
	{
		char szWarning[] = "ÇëÄú²»ÒªÔÚÏûÏ¢ÖÐÊ¹ÓÃ²»ºÏÊÊµÄ´ÊÓï!"; 
...
void KUiPlayerBar::OnSendChannelMessage(DWORD nChannelID, const char* Buffer, int nLen)				//·¢ËÍÆµµÀÁÄÌìµ½·þÎñÆ÷
{
	if (nChannelID != -1 && Buffer && nLen > 0)
	{
		...
		if (nLeft < 0)
		{
			char szWarning[] = "Kh«ng thÓ sö dông thiªn lý truyÒn ©m liªn tôc, xin h·y ®îi gi©y l¸t !";
			KUiMsgCentrePad::SystemMessageArrival(szWarning, sizeof(szWarning));
		}
		else if (nLeft > 0)
		{
	...
	while(KUiMsgCentrePad::GetChannelData(dwID, cost, Buffer, nLen))
	{
		...
		if (!bSend)
		{
			...
			if (nRet == 1 && nUseMoney > 0)
				sprintf(szSystem, "Sö dông thiªn lý truyÒn ©m cÇn %d ng©n l­îng !", nUseMoney);

			if (nRet == 3 && nUseMana > 0)
				sprintf(szSystem, "Sö dông thiªn lý truyÒn ©m cÇn %d néi lùc !", nUseMana);
		}
		else
		{
			if (nUseMoney > 0)
			...
			if (nUseMana > 0)
				sprintf(szSystem, "B¹n sö dông thiªn lý truyÒn ©m tiªu hao %d néi lùc !", nUseMana);
		}
