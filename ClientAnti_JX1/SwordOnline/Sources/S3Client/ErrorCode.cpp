...
#define ERROR_STRING		"\\UI\\LoiGame.ini"
...
void Error_Box()
{
	if(s_uErrorCode)
	{
		...
		if(szFormatString[0] == 0)
		{
			...
			if(szFormatString[0] == 0)
			    strcpy(szFormatString, "Phat sinh loi(Thong tin: %d)");
		}
		while(true)
		{
	    ...
	    MessageBox(NULL, szOutputInfo, "Loi khi khoi dong game", MB_OK | MB_ICONERROR);
