...
public:
	...
	~KSkillList();
	void		ReEnChance();
	int			Add(int nSkillID, int nLevel = 1, int nMaxTimes = 0, int RemainTimes = 0, int nAdd = 0);
...
#ifndef _SERVER
	void		SetAddPoint(int nId, int nLevel);
	void		SetEnChance(int nId, int nLevel);
	...
	int			GetAddPoint(int nListIndex)
	{
		if (nListIndex < MAX_NPCSKILL && nListIndex > 0)
			return m_Skills[nListIndex].AddPoint;
		else 
			return 0;
	};

	int			GetEnChance(int nListIndex)
	{
		if (nListIndex < MAX_NPCSKILL && nListIndex > 0)
	...
	void		QeuipAddPoint(int nListIndex,int add)
	{
			m_Skills[nListIndex].AddPoint += add;
	};

	void		AddEnChance(int nListIndex,int add)
	{
