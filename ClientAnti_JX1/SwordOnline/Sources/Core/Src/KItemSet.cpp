...
int KItemSet::Add(int nItemGenre, int nSeries, 
				  ...
				  int nParticularType/*=-1*/, int* pnMagicLevel, int nVersion/*=0*/, UINT nRandomSeed,int nStackNum, int nEnChance,
				  int nPoint, int nYear, int nMonth, int nDay, int nHour)
{
	int i = FindFree();
	...
	case item_medicine:			// 药品类
		...
		break;
	case item_mine:				// 矿石类
		ItemGen.Gen_Mine(nParticularType, pItem, nStackNum, nLevel, nSeries,nLuck);
		break;
