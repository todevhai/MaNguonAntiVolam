...
m_nValue1 ±íÊ¾×Óµ¯Ö®¼äµÄ¾àÀë£¬µ¥Î»ÏñËØµã
*/
int KSkill::CastNotWall(TOrdinSkillParam * pSkillParam,  int nDir , int nRefPX , int nRefPY)  const 
{
	int nLauncher = pSkillParam->nLauncher;
	eSkillLauncherType eLauncherType = pSkillParam->eLauncherType;
	if (eLauncherType != SKILL_SLT_Npc) return 0;
...
KMissleMagicAttribsData* KSkill::CreateMissleMagicAttribsData(int nLauncher)  const 
{
	...
	if (nLauncher)
	{
		Npc[nLauncher].AppendSkillEffect(m_nIsMagic,m_bIsPhysical, m_bIsMelee, (KMagicAttrib *)m_DamageAttribs, pDamageAttribs, m_nEnChance);
	}
	else
	{
...
lab_processdamage:			
	...
	if (pAttribsData) 
	{
		if (Npc[nParam2].ReceiveDamage(nLauncher, m_bIsMelee, pAttribsData->m_pDamageMagicAttribs, m_bUseAttackRate, m_bDoHurt,m_nEnChance))
		{		
...
void KSkill::EnChanceSkill(int nLauncher)
{
	...
	for(int i = 0;i < m_nImmediateAttribsNum;i++)
	{
		if (m_ImmediateAttribs[i].nAttribType == magic_addskilldamage1 || 
			m_ImmediateAttribs[i].nAttribType == magic_addskilldamage2)
		{
...
void	KSkill::GetDesc(unsigned long ulSkillId, unsigned long ulCurLevel, char * pszMsg, int nOwnerIndex,  bool bGetNextLevelDesc, int nAddPoint,int nEnChance)
{
	...
	case MISSLE_MMK_Parabola:
		{
			strcat(pszMsg, "MMK_Parabola ");
		}break;						//	Å×ÎïÏß
	case MISSLE_MMK_SingleLine:{}break;						//	±ØÖÐµÄµ¥Ò»Ö±Ïß·ÉÐÐÄ§·¨
	}
	
	if (!g_MisslesLib[pTempSkill->m_nChildSkillId].m_bCollideVanish)
		strcat(pszMsg, "m_bCollideVanish");
	if (g_MisslesLib[pTempSkill->m_nChildSkillId].m_nDamageRange > 1) 
	{
		sprintf(szTemp, "m_nDamageRange:%d ", g_MisslesLib[pTempSkill->m_nChildSkillId].m_nDamageRange);
		strcat(pszMsg, szTemp);
	}
	if (g_MisslesLib[pTempSkill->m_nChildSkillId].m_nKnockBack)
	{
		sprintf(szTemp, "m_nKnockBack:%d ", g_MisslesLib[pTempSkill->m_nChildSkillId].m_nKnockBack);
		strcat(pszMsg, szTemp);
	}
	if (g_MisslesLib[pTempSkill->m_nChildSkillId].m_bAutoExplode)
	{
		strcat(pszMsg, "m_bAutoExplode ");
	}
	
	if (pTempSkill->m_bIsAura) strcat(pszMsg, "m_bIsAura");
	if (pTempSkill->m_bIsPhysical) strcat(pszMsg, "m_bIsPhysical");
	if (pTempSkill->m_bIsMelee) strcat(pszMsg, "m_bIsMelee");
	if (pTempSkill->m_bTargetOnly) strcat(pszMsg, "m_bTargetOnly");
	if (pTempSkill->m_bTargetAlly) strcat(pszMsg, "m_bTargetAlly");
	if (pTempSkill->m_bTargetEnemy) strcat(pszMsg, "m_bTargetEnemy ");
	if (pTempSkill->m_bTargetObj)	  strcat(pszMsg, "m_bTargetObj ");
	if (pTempSkill->m_bTargetSelf) strcat(pszMsg, "m_bTargetSelf ");
	if (pTempSkill->m_bUseAttackRate) strcat(pszMsg, "m_bUseAttackRate ");
#endif
	...
	if (pTempSkill->IsMagic() && (pTempSkill->m_eSkillStyle == SKILL_SS_Melee || pTempSkill->m_eSkillStyle == SKILL_SS_Missles))
	{
		strcat(pszMsg, "Vâ c«ng l­u ph¸i: <color=Fire>Néi c«ng<color>\n");
	}
	else if (pTempSkill->m_eSkillStyle == SKILL_SS_Melee || pTempSkill->m_eSkillStyle == SKILL_SS_Missles)
	{
		strcat(pszMsg, "Vâ c«ng l­u ph¸i: <color=Fire>Ngo¹i c«ng<color>\n");
	}
	strcat (pszMsg, "\n");
	   ...
	   nay - du lieu ba ban deu ghi IsPhysical=1 cho id 14. */
	{
		if (nAddPoint)
		{
			sprintf(szTemp, "§¼ng cÊp hiÖn t¹i: %d <color=BLUE>(%d + %d)<color>",ulCurLevel , ulCurLevel - nAddPoint,nAddPoint);
		} 
		else
		{
	...
	if (nEnChance)
	{
		...
		sprintf(szTemp, "Hç trî tõ kü n¨ng kh¸c + <color=Fire>%d%s<color>", nEnChance,"%");
		strcat(pszMsg, "\n");
		strcat(pszMsg, szTemp);
		strcat(pszMsg, "\n");
	}
	...
	if (-2 !=pTempSkill->m_nEquiptLimited)
	{
		...
		{
		case -1:
			strcat(pszMsg, "<color=Cyan>Tay kh«ng<color>\n");
			break;
		case 0:
			strcat(pszMsg, "<color=Cyan>KiÕm <color>\n");
			break;
		case 1:
			strcat(pszMsg, "<color=Cyan>§¬n ®ao<color>\n");
			break;
		case 2:
			strcat(pszMsg, "<color=Cyan>C«n - bæng<color>\n");
			break;
		case 3:
			strcat(pszMsg, "<color=Cyan>Th­¬ng - kÝch<color>\n");
			break;
		case 4:
			strcat(pszMsg, "<color=Cyan>Chïy<color>\n");
			break;
		case 5:
			strcat(pszMsg, "<color=Cyan>Song ®ao<color>\n");
			break;
		case 101:
			strcat(pszMsg, "<color=Cyan>Phi ®ao<color>\n");
			break;	
		case 102:
			strcat(pszMsg, "<color=Cyan>Tô tiÔn<color>\n");
			break;
		case 100:
			strcat(pszMsg, "<color=Cyan>Phi tiªu<color>\n");
			break;
		}
	}
	else
	{
		strcat(pszMsg, "<color=Blue>Kh«ng h¹n chÕ <color>\n");
	}
	if (pTempSkill->m_nHorseLimited)
	{
		...
		case 1:
			{
				strcat(pszMsg, "<color=Red>Kh«ng thÓ sö dông kü n¨ng khi c­ìi ngùa<color>\n");
			}
			break;
		case 2:
			{
				strcat(pszMsg, "Kü n¨ng cã thÓ sö dông khi c­ìi ngùa\n");
			}
			break;
		default:
			return ;
...
void KSkill::GetDescAboutLevel(char * pszMsg)
{
	...
	if (nGetCost)
	{
		...
		case attrib_mana:
			...
			sprintf(szTemp, "Néi lùc mÊt ®i: %d\n", nGetCost);
			strcat(pszMsg,szTemp);
			break;
		case attrib_stamina:
			sprintf(szTemp, "ThÓ l­c mÊt ®i: %d\n", nGetCost);
			strcat(pszMsg,szTemp);
			break;
		case attrib_life:
			sprintf(szTemp, "Sinh lùc mÊt ®i: %d\n", nGetCost);
			strcat(pszMsg,szTemp);
			break;
		}
	}
	...
	for (i  = 0; i < m_nImmediateAttribsNum; i ++)
	{
		if (/*m_bCollideEvent &&*/ m_ImmediateAttribs[i].nAttribType == magic_skill_collideevent && m_ImmediateAttribs[i].nValue[0])
		{
			...
			if (m_bByMissle && !nCount)
				strcat(pszMsg, "<color=blue>TÇng thø 2: <color>");
			else
				strcat(pszMsg, "<color=blue>TÇng thø 3: <color>");
...
BOOL KSkill::CastStateSkill( int nLauncher, int nParam1, int nParam2, int nWaitTime ) const
{
	...
	if (m_nStateAttribsNum > 0)
	{
		Npc[nLauncher].SetStateSkillEffect(nLauncher, m_nId, m_ulLevel, (KMagicAttrib *)m_StateAttribs, m_nStateAttribsNum, nWaitTime);
	}
#endif
	return TRUE;
}
