...
NPC_RELATION KNpcSet::GetRelation(int nId1, int nId2)
{
	...
	else if (Player[CLIENT_PLAYER_INDEX].m_nIndex == nId1)
	{
		...
		if (Player[CLIENT_PLAYER_INDEX].m_cPK.GetEnmityPKState() == enumPK_ENMITY_STATE_PKING &&
			...
			return relation_enemy;
		if (Npc[nId2].m_Kind == kind_player && Npc[nId2].m_nPKFlag != 2 && Player[CLIENT_PLAYER_INDEX].m_cPK.GetNormalPKState() == FALSE)
			return relation_none;
		if (Npc[nId2].m_Kind == kind_player && !Npc[nId2].m_nPKFlag && Player[CLIENT_PLAYER_INDEX].m_cPK.GetNormalPKState() != 2)
			return relation_none;
		return (NPC_RELATION)m_RelationTable
			[Npc[nId1].m_Kind]
			[Npc[nId2].m_Kind]
	...
	else	// if (Player[CLIENT_PLAYER_INDEX].m_nIndex == nId2)
	{
		...
		if (Npc[nId1].m_Kind == kind_player && Npc[nId2].m_Kind == kind_player && !Npc[nId1].m_nPKFlag)
			return relation_none;
		if (Npc[nId1].m_Kind == kind_player && Npc[nId2].m_Kind == kind_player && Npc[nId1].m_nPKFlag == 1 && Player[CLIENT_PLAYER_INDEX].m_cPK.GetNormalPKState() == FALSE)
			return relation_none;
		return (NPC_RELATION)m_RelationTable
			[Npc[nId1].m_Kind]
			[Npc[nId2].m_Kind]
...
//			return relation_none;
		...
		{
			if (!Player[Npc[nId1].m_nPlayerIdx].m_cPK.GetNormalPKState() && Player[Npc[nId2].m_nPlayerIdx].m_cPK.GetNormalPKState() != 2)
				return relation_none;
			if (!Player[Npc[nId2].m_nPlayerIdx].m_cPK.GetNormalPKState() && Player[Npc[nId1].m_nPlayerIdx].m_cPK.GetNormalPKState() != 2)
				return relation_none;
		}
		return nRelation;
	}
