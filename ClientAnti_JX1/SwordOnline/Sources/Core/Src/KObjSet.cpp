...
int		KObjSet::AddMoneyObj(KMapPos MapPos, int nMoneyNum)
{
	...
	if (nMoneyNum > 10000)
	{
		sprintf(szMoney,"%d v筺 %d lng",nMoneyNum / 10000,nMoneyNum % 10000);
	}
	else
	{
		sprintf(szMoney,"%d lng",nMoneyNum);
	}
	strcpy(sInfo.m_szName,szMoney);
	sInfo.m_nColorID = 0;
...
int		KObjSet::Add(int nDataID, KMapPos MapPos, KObjItemInfo sItemInfo)
{
	...
	{
		if (Item[sItemInfo.m_nItemID].GetGoldId() > 0)        // 黄金装备
		{
			sItemInfo.m_nColorID = 3;
		}
		else if (Item[sItemInfo.m_nItemID].IsPurple())
		{
			sItemInfo.m_nColorID = 4;
		}
		else if (Item[sItemInfo.m_nItemID].GetAttribType())        // 魔法装备
		{
    ...
    else if(Object[nAddNo].m_nKind == Obj_Kind_Box || Object[nAddNo].m_nKind == Obj_Kind_Money)
	{
		sItemInfo.m_nColorID = 3;
	}
