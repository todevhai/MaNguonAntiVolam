...
enum GAMEDATA_INDEX
{
	...
	GDI_SELL_ITEM_NUM, // truyen so luong item hanh trang
...
//	GOI_DROP_ITEM,				//放置物品到游戏窗口
	...
	GOI_PLAYER_SYSSHOP,//奇珍阁

	GOI_LIXIAN,

	GOI_GIVE_ITEM_BACK,//uparam = item nparam = num
...
struct _declspec (novtable) iCoreShell
{
	...
	virtual int GetGenreItem(unsigned int UId) = 0;
	virtual	int GetTypeItem(unsigned int UId) = 0;
	...
	virtual	int GetTradeItem(unsigned int UId) = 0;
	virtual	int GetDropItem(unsigned int UId) = 0;
};
