...
int	g_nProtocolSize[MAX_PROTOCOL_NUM] = 
{
	...
	sizeof(PLAYER_XU_SYNC),					// s2c_syncxu
	...
	sizeof(STRING_BOX),						// s2c_stringbox
	...
	sizeof(CURPLAYER_TASKINFO_SYNC),			//s2c_synccurplayerinfo
	sizeof(NPC_SIT_SYNC),
	...
	sizeof(PLAYER_REQUEST_OPEN_MARKET),			// c2s_market
	sizeof(CP_STRING),							// c2s_string
	...
	sizeof(PLAYER_COMPITEM),					// c2s_playercomp
...
void SendClientCmdBreak(DWORD dwID, int nNum)
{
...
void SendClientCmdGive( unsigned int uId, int nItemNum, int nKind )
{
...
void SendClientCmdComp( unsigned int uId, int nItemNum, int nKind )
{
	unsigned int* pInfo = (unsigned int*) uId;
	...
	PlayerComp.ProtocolType = c2s_playercomp;
	...
	for (int i = 0;i < nItemNum;i++)
	{
		...
		PlayerComp.uId[i] = Item[nIdx].GetID();
		temp[i]=nIdx;
		pInfo++;
	}
