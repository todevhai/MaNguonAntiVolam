...
KNpcAttribModify::KNpcAttribModify()
{
	...
	ProcessFunc[magic_addphysicsmagic_v] = &KNpcAttribModify::AddPhysicsMagic;
	ProcessFunc[magic_addcoldmagic_v] = &KNpcAttribModify::AddColdMagic;
	ProcessFunc[magic_addfiremagic_v] = &KNpcAttribModify::AddFireMagic;
	ProcessFunc[magic_addlightingmagic_v] = &KNpcAttribModify::AddLightingMagic;
	ProcessFunc[magic_addpoisonmagic_v] = &KNpcAttribModify::AddPoisonMagic;
	...
	ProcessFunc[magic_mintimepercastonhorse_v] = &KNpcAttribModify::mintimepercastonhorseV;
	...
	ProcessFunc[magic_enhance_709_auto] = &KNpcAttribModify::enhance_709_auto;
	ProcessFunc[magic_enhance_708_life_p] = &KNpcAttribModify::enhance_708_lifeP;
	ProcessFunc[magic_enhance_93_life_v] = &KNpcAttribModify::enhance_93_lifeV;
...
void KNpcAttribModify::AllSkillV(KNpc* pNpc, void* pData)
{
	...
	if (pMagic->nValue[2] != 0)
	{
		...
		if (pMagic->nValue[2] > 0)
			...
			Same = -pMagic->nValue[2];
		if (pNpc->m_SkillList.FindSame(Same))
		{
			if (pNpc->m_SkillList.GetCurrentLevel(Same) > 0)
			{
				pNpc->m_SkillList.QeuipAddPoint(pNpc->m_SkillList.FindSame(Same),pMagic->nValue[0]);
#ifdef _SERVER
				pNpc->m_SkillList.IncreaseLevel(pNpc->m_SkillList.FindSame(Same),pMagic->nValue[0],TRUE);
				...
				sSkill.m_nAddPoint = pNpc->m_SkillList.GetAddPoint(pNpc->m_SkillList.FindSame(Same));
...
#ifdef _SERVER
					pNpc->m_SkillList.IncreaseLevel(i,pMagic->nValue[0],TRUE);
					// 把当前此技能的点数和剩余技能点发给客户端
					PLAYER_SKILL_LEVEL_SYNC	sSkill;
					sSkill.ProtocolType = s2c_playerskilllevel;
					sSkill.m_nSkillID = pNpc->m_SkillList.GetSkillId(i);
					sSkill.m_nSkillLevel = pNpc->m_SkillList.GetCurrentLevel(pNpc->m_SkillList.GetSkillId(i));
					sSkill.m_nLeavePoint = Player[pNpc->m_nPlayerIdx].m_nSkillPoint;
					sSkill.m_nAddPoint = pNpc->m_SkillList.GetAddPoint(i);
					g_pServer->PackDataToClient(Player[pNpc->m_nPlayerIdx].m_nNetConnectIdx, (BYTE*)&sSkill, sizeof(PLAYER_SKILL_LEVEL_SYNC));
#endif
				}
			}
...
void KNpcAttribModify::AddPhysicsMagic( KNpc* pNpc, void* pData )
{
	...
	g_DebugLog("[数值]%s普伤害增加(%d)", pNpc->Name, pMagic->nValue[0]);
}
...
//				pNpc->m_CurrentStaminaMax += pMagic->nValue[1];
				pNpc->m_SkillList.AddEnChance(pNpc->m_SkillList.FindSame(Same),pMagic->nValue[2]);
			}			
		}
	}
...
void KNpcAttribModify::poison2decmanaP( KNpc* pNpc, void* pData )
{
	...
	if (pNpc->m_LoseMana.nTime <= 0)
	{
		if (pMagic->nValue[1] > 0 && pMagic->nValue[0] > 0)
		{
			...
			pNpc->m_LoseMana.nValue[1] = pMagic->nValue[1] * 2;
		}
	}
...
void KNpcAttribModify::skill_appendskil( KNpc* pNpc, void* pData )
{
	...
	if (pMagic->nValue[0] > 0)
	{
		for (int i = 0;i < 5;i++)
		{
			if (pNpc->m_TmpAuraID[i] == pMagic->nValue[0])
				break;
			if (!pNpc->m_TmpAuraID[i])
			{
				pNpc->m_TmpAuraID[i] = pMagic->nValue[0];
				break;
			}
		}
	}
...
void KNpcAttribModify::enhance_709_auto( KNpc* pNpc, void* pData )
{
	
}

void KNpcAttribModify::enhance_708_lifeP( KNpc* pNpc, void* pData )
{
	
}

void KNpcAttribModify::enhance_93_lifeV( KNpc* pNpc, void* pData )
{
...
void KNpcAttribModify::autoskill( KNpc* pNpc, void* pData )
{
	KMagicAttrib* pMagic = (KMagicAttrib *)pData;
#ifdef _SERVER
	pNpc->m_nAutoSkillId = pMagic->nValue[0];
	if (pMagic->nValue[1] == -1)
		pNpc->m_nAutoLevel = 1;
	else
		pNpc->m_nAutoLevel = pMagic->nValue[1];
	
	pNpc->m_nAutoRate = pMagic->nValue[2];
