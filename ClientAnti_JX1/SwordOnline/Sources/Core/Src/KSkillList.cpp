...
BOOL KSkillList::IncreaseLevel(int nIdx, int nLvl, BOOL Qeuip)
{
...
int KSkillList::Add(int nSkillID, int nLevel, int nMaxTimes, int RemainTimes, int nAdd)
{
...
//			{
//				((KSkill *)pSkill)->EnChanceSkill(m_nNpcIndex);
//			}
		}
...
void KSkillList::SetAddPoint( int nId, int nLevel )
{
	int i = FindSame(nId);

	if (i > 0 && i < MAX_NPCSKILL)
	{
		m_Skills[i].AddPoint = nLevel;
	}
}

void KSkillList::SetEnChance( int nId, int nLevel )
{
	int i = FindSame(nId);
	
	if (i > 0 && i < MAX_NPCSKILL)
	{
		m_Skills[i].EnChance = nLevel;
	}
}
#endif
...
int	KSkillList::UpdateDBSkillList(BYTE * pSkillBuffer)
{
	...
	for (int i = 0 ; i < MAX_NPCSKILL; i ++)
	{
		if (m_Skills[i].SkillId > 0)
		{
				...
				if (m_Skills[i].AddPoint > 0)
					pSkillData->m_nSkillLevel = m_Skills[i].SkillLevel - m_Skills[i].AddPoint;		
				else
		...
		if (m_Skills[i].SkillId > 0)
		{
			...
			if (pSkill)
			{
				if (pSkill->IsPhysical())
					continue;
				nTotalSkillPoint += m_Skills[i].SkillLevel - m_Skills[i].AddPoint;
				m_Skills[i].SkillLevel = 0;
				m_Skills[i].CurrentSkillLevel = 0;
				m_Skills[i].AddPoint = 0;
				m_Skills[i].EnChance = 0;
			}
		}
	}
