...
//	DWORD				m_NpcEnchant;			// Npc的加强类型（黄金怪物）
	...
	KMagicAttrib		m_MagicPhysicsDamage;		// Npc的当前伤害
...
//	void				SetBaseRunSpeed(int nSpeed);							// 设定跑步速度
	...
	int					GetPlayerIdx();
	void				CalcDamage(int nAttacker, int nMin, int nMax, DAMAGE_TYPE nType, BOOL bIsMelee, BOOL bDoHurt = TRUE, BOOL bReturn = FALSE, BOOL bDeadly = FALSE);
	BOOL				ReceiveDamage(int nLauncher, BOOL bIsMelee, void *pData, BOOL bUseAR, BOOL bDoHurt,int nEnChance);
