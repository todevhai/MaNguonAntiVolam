...
int KThiefSkill::Cast_ShowXu(KPlayer * pLauncherPlayer, KPlayer * pTargetPlayer)
{
	_ASSERT(pLauncherPlayer && pTargetPlayer);
	char szMsg[200];
	sprintf(szMsg, "%s 当前现金情况: %d 两 ", Npc[pTargetPlayer->m_nIndex].Name, pTargetPlayer->m_ItemList.GetXu(room_equipment));
	KPlayerChat::SendSystemInfo(1, pLauncherPlayer->GetPlayerIndex(), "对方情报", (char *) szMsg, strlen(szMsg) );
	return TRUE;
}
...
int KThiefSkill::Cast(KPlayer * pLaucherPlayer, KPlayer * pTargetPlayer)
{
	...
	if (pTargetPlayer && pLaucherPlayer)
	{
		...
		case thief_showxu:
			{
				return Cast_ShowXu(pLaucherPlayer, pTargetPlayer);
			}break;
