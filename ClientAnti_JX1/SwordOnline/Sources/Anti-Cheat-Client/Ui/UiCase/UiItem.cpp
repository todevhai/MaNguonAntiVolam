...
void KUiItem::LoadScheme(const char* pScheme)
{
	...
	if (m_pSelf && Ini.Load(Buff))
	{
		...
		m_pSelf->m_Xu.Init(&Ini, "GoldCoin");
		m_pSelf->m_MoneyIcon.Init(&Ini, "MoneyIcon");
		m_pSelf->m_XuIcon.Init(&Ini, "XuIcon");
...
void KUiItem::OnSetItem(KUiDraggedObject* pItem ,int nPrice)
{
	...
	g_pCoreShell->OperationRequest(GDI_SET_TRADE_ITEM,
					(unsigned int)(&Obj), nPrice);
...
void KUiItem::OnClickItem(KUiDraggedObject* pItem, bool bDoImmed)
{
	...
	else
	{
		if (KUiComItem::GetIfVisible())
		{
			...
			g_pCoreShell->OperationRequest(GOi_SET_WINDOWS_ITEM,(unsigned int)pItem, pCom->GetWindowsNum());					
		m_ItemBox.RemoveObject(pItem);
		} 
		else
		{
		
			UISYS_STATUS eStatus = g_UiBase.GetStatus();
			if (eStatus == UIS_S_TRADE_SALE || eStatus == UIS_S_TRADE_NPC && !m_pSelf->m_MakeStallBtn.IsButtonChecked())
			{
...
int KUiItem::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)
{
	...
	case WND_N_LEFT_CLICK_ITEM:
		...
		else if (g_UiBase.GetStatus() == UIS_S_TRADE_NPC && !m_MakeStallBtn.IsButtonChecked())
	...
	case WND_N_BUTTON_CLICK:
		...
		{
			if (!KUiStoreBox::GetIfVisible())
				KUiGetString::OpenWindow("Nh藀 l阨 rao","",(KWndWindow*)this,UIITEM_WAIT_GETNAME,1,16);
		}
			...
			
		{
			if (g_pCoreShell)
			{
				if(g_pCoreShell->OperationRequest(GDI_PLAYER_TRADE,
									(unsigned int)(&m_ShopName), 0) == 
					0)	
				{
					m_MakeStallBtn.CheckButton(FALSE);
					break;
