...
void KUiTongAssignBox::LoadScheme(const char* pScheme)
{
	if(ms_pSelf)
	{
		...
		if(Ini.Load(Buff))
		{
			...
			ms_pSelf->m_BtnPilgarlic.SetText("Bang chóng");
			...
			Ini.GetString("Main", "ErrorFigure", "Xin mêi lùa chän chøc vÞ", ms_pSelf->m_szErrorNotSelectFigure, sizeof(ms_pSelf->m_szErrorNotSelectFigure));
		}
	}
}
