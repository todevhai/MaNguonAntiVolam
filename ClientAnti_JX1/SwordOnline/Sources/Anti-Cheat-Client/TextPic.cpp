...
#include "KIniFile.h"
#include "CrtDbg.h"
//#include "Represent2/KRepresentShell2.h"
...
#define CHAR_BUFFER_LEN	32
	...
	for (i = 0; i < nPicCount; i++)
	{
		...
		//HookGetImageParam(0x6DE30C, CurrentImg.szImage, &Param, CurrentImg.nType);
		g_pRepresentShell->GetImageParam(CurrentImg.szImage, &Param, CurrentImg.nType);
...
long KInlinePicSink::DrawPic(unsigned short wIndex, int x, int y)
{
	...
	//HookDrawPrimitives(0x6DE30C, 1, pPic, RU_T_IMAGE, true);
	g_pRepresentShell->DrawPrimitives(1, pPic, RU_T_IMAGE, true);
	IR_NextFrame(*pPic);
	return S_OK;
}
...
long KInlinePicSink::AddCustomInlinePic(unsigned short& rwIndex, const char* szSprPathName)
{
	...
	CurrentImg.nType = ISI_T_SPR;
	memset(&Param, 0, sizeof(Param));
	//if (!HookGetImageParam(0x6DE30C, CurrentImg.szImage, &Param, CurrentImg.nType))
	if (!g_pRepresentShell->GetImageParam(CurrentImg.szImage, &Param, CurrentImg.nType))
		return E_FAIL;
