...
#define	UI_PUBLIC_FONT			"PublicSetting.ini"			//某个界面方案的设定文件
...
int KUiBase::LoadScheme(const char* pScheme)
{
	...
	sprintf(Buffer, "%s\\%s", UI_PUBLIC_FONT, m_CurSchemePath);
	if (GetSchemePath(pScheme))
	{
		...
		char		Section[8];
		//----卸载字体----
		if (g_pRepresent && Ini.Load(Buffer) &&
		...
		//----载入字体----
		if (g_pRepresent && Ini.GetInteger(FONT_SECTION, "Count", 0, &nCount))
		{
			for (i = 0; i < nCount; i++)
			{
				itoa(i, Section, 10);
				if (Ini.GetInteger(FONT_SECTION, Section, 0, &nId))
				{
					...
					if (Ini.GetString(FONT_SECTION, Section, "", Buffer, sizeof(Buffer)) &&
							Buffer[0])
					{
						g_pRepresent->CreateAFont(Buffer, CHARACTER_CODE_SET_GBK, nId);
					}
				}
			}			
		}
