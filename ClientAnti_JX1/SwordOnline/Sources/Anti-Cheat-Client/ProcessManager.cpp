...
bool CProcessManager::CheckProcess(DWORD flag) // OK
{
	...
	for(int n=0,ProcessCount=(BytesReturned/sizeof(DWORD));n < ProcessCount;n++)
	{
		...
		if(ProcessIds[n] != 0)
		{
			if(this->AddProcessCache(ProcessIds[n], flag) != 0)
			{
...
bool CProcessManager::AddProcessCache(DWORD ProcessId, DWORD flag)
{
	if (gProcessCheck.CheckProcess(ProcessId, flag) == 0)
	{
		return 0;
	}
