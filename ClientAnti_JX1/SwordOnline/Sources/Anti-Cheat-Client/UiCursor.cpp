    ...
    #else
	{
		if (g_pRepresent)
		{
			for (int i = 0; i < MAX_CURSOR_IMAGE; i++)
			{
				...
				{
					g_pRepresent->FreeImage(m_CursorImages[i].szImage);
...
void KUiCursor::SetImage(int nIndex, const char* pImgFile)
{
	...
	{	
	    if (nIndex >= 0 && nIndex < MAX_CURSOR_IMAGE && pImgFile)
		{
			...
			g_pRepresent->GetImageParam(pCurrentImg->szImage, &Param, pCurrentImg->nType);
...
void KUiCursor::Paint()
{
    ...
    #else   // KUI_USE_HARDWARE_MOUSE
	{
	    if (m_bShow && g_pRepresent)
		{
			...
			g_pRepresent->DrawPrimitives(1, &m_CursorImages[m_nCurImage], RU_T_IMAGE, true);
