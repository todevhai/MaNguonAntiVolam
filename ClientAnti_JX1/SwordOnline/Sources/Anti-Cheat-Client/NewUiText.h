...
public:
	KWndText() {}
	//virtual int		Init(KIniFile* pIniFile, const char* pSection) { return KWndText32_Init(this, pIniFile, pSection); }
	virtual int		Init(KIniFile* pIniFile, const char* pSection) { return 0; }
	virtual void	PaintWindow() {}
	int				GetText(char* pBuffer, int nSize) { return 0; }
	...
	void            SetNumberItem(int nNumber, char Separator = 0) {}
	void			Set4IntText(int nNumber, char Separator = 0) {}
	void			Set5IntText(int nNumber, char Separator = 0) {}
	void            SetXuText(int nNumber, char Separator = 0) {}
	//设置文本串的内容为与表述所给两个整数的字符串，两个整数间以指定的字符分割
	void			Set2IntText(int nNumber1, int nNumber2, char Separator) {}
	int				SetTopLine(int nTopLine) { return 0; }
...
protected:
	...
	void			SetAttackText(int nNumber1, int nNumber2, char Separator = 0) {}	//hien thi toc do danh ngoai noi
...
private:
	...
	BYTE			vptr[184];
public:
	KWndText32() { SetObjectWndText32((unsigned int*)this); }
};
