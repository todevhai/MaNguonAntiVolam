...
bool CFileProtect::EncryptFile(char* path) // OK
{
	...
	for(int n=0;n < ((int)this->m_size);n++)
	{
		...
		buff[sizeof(gFileProtectHeader)+n] += gFileProtectXorTable[gProtect.m_MainInfo.ServerName[n%sizeof(gProtect.m_MainInfo.ServerName)]%sizeof(gFileProtectXorTable)];
	}
...
bool CFileProtect::DecryptFile(char* path) // OK
{
	...
	{
		this->m_buff[sizeof(gFileProtectHeader)+n] -= gFileProtectXorTable[gProtect.m_MainInfo.ServerName[n%sizeof(gProtect.m_MainInfo.ServerName)]%sizeof(gFileProtectXorTable)];
