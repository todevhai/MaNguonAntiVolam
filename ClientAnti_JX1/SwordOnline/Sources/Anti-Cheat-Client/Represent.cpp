...
Represent::Represent(KCanvas* m_Canvas, KDirectDraw* m_DirectDraw)
{
	//HookVTableFunction((void*)0x6DE30C, (void)&OutputText, 4);
	//HookVTableFunction((void*)0x6DE30C, (int)&OutputRichText, 5);
	//HookVTableFunction((void*)0x6DE30C, (void)&DrawPrimitives, 18);
	m_CanvasLocal = m_Canvas;
	m_DirectDrawLocal = m_DirectDraw;
	m_nLeft = 0;
	m_nTop = 0;
...
bool Represent::Create(int nWidth, int nHeight, bool bFullScreen)
{
...
HRESULT Represent::AdviseRepresent(IInlinePicEngineSink* pSink)
{
...
HRESULT Represent::UnAdviseRepresent(IInlinePicEngineSink* pSink)
{
...
//设置偏色列表
unsigned int Represent::SetAdjustColorList(unsigned int* puColorList, unsigned int uCount)
{
	return m_ImageStore.SetAdjustColorList(puColorList, uCount);
}

//##ModelId=3DCA0B230317
bool Represent::CreateAFont(const char* pszFontFile, CHARACTER_CODE_SET CharaSet, int nId)
{
	int nFirstFree = -1;
	for (int i = 0; i < RS2_MAX_FONT_ITEM_NUM; i++)
	{
...
unsigned int Represent::CreateImage(const char* pszName, int nWidth, int nHeight, int nType)
{
	return m_ImageStore.CreateImage(pszName, nWidth, nHeight, nType);
}

//##ModelId=3DB69FE401DA
void Represent::DrawPrimitives(int nPrimitiveCount, KRepresentUnit* pPrimitives, unsigned int uGenre, int bSinglePlaneCoord)
{
	int i = 0;
...
void Represent::DrawPrimitivesOnImage(int nPrimitiveCount, KRepresentUnit* pPrimitives,
...
//##Documentation
//## 清除图形数据
void Represent::ClearImageData(const char* pszImage, unsigned int uImage, short nImagePosition)
{
	void* pFrame;
	KSGImageContent* pBitmap = (KSGImageContent*)m_ImageStore.GetImage(
...
void Represent::FreeAllImage()
{
	m_ImageStore.Free();
}

//##ModelId=3DCD8EF60316
void Represent::FreeImage(const char* pszImage)
{
	m_ImageStore.FreeImage(pszImage);
}

//##ModelId=3DCD8FA900EE
void* Represent::GetBitmapDataBuffer(const char* pszImage, KBitmapDataBuffInfo* pInfo)
{
...
void Represent::ReleaseBitmapDataBuffer(const char* pszImage, void* pBuffer)
{
	unsigned int uImage = 0;
	short		nISPosition = -1;
	LPDIRECTDRAWSURFACE pSurface;
...
bool Represent::GetImageParam(const char* pszImage, KImageParam* pImageData, int nType)
{
	return m_ImageStore.GetImageParam(pszImage, nType, pImageData);
}

bool Represent::GetImageFrameParam(const char* pszImage, int nFrame,
...
int Represent::GetImagePixelAlpha(const char* pszImage, int nFrame, int nX, int nY, int nType)
{
	return m_ImageStore.GetImagePixelAlpha(pszImage, nType, nFrame, nX, nY);
}

//##ModelId=3DC0A08D0085
void Represent::LookAt(int nX, int nY, int nZ)
{
...
void Represent::OutputText(int nFontId, const char* psText, int nCount, int nX, int nY, unsigned int Color, int nLineWidth, int nZ, unsigned int BorderColor)
{
	for (int i = 0; i < RS2_MAX_FONT_ITEM_NUM; i++)
	{
...
//##Documentation
//## 输出文字。
int Represent::OutputRichText(int nFontId, KOutputTextParam* pParam,
...
//## 返回指定坐标在字符串中最近的字符偏移
int Represent::LocateRichText(int nX, int nY,
...
void Represent::ReleaseAFont(int nId)
{
	for (int i = 0; i < RS2_MAX_FONT_ITEM_NUM; i++)
	{
...
bool Represent::Reset(int nWidth, int nHeight, bool bFullScreen)
{
	return Create(nWidth, nHeight, bFullScreen);
}

//##ModelId=3DCD90910361
bool Represent::SaveImage(const char* pszFile, const char* pszImage, int nFileType)
{
	return m_ImageStore.SaveImage(pszFile, pszImage, nFileType);
}

//##ModelId=3DCD90F30011
void Represent::SetImageStoreBalanceParam(int nNumImage, unsigned int uCheckPoint)
{
	m_ImageStore.SetBalanceParam(nNumImage, uCheckPoint);
}

//##ModelId=3DD00EEE0149
bool Represent::CopyDeviceImageToImage(const char* pszName, int nDeviceX, int nDeviceY, int nImageX, int nImageY, int nWidth, int nHeight)
{
...
void Represent::CoordinateTransform(int& nX, int& nY, int nZ)
{
	nX = nX - m_nLeft;
...
bool Represent::RepresentBegin(int bClear, unsigned int Color)
{
	KRColor	c;
	c.Color_dw = Color;
	if (bClear)
...
//视图/绘图设备坐标 转化为空间坐标
void Represent::ViewPortCoordToSpaceCoord(int& nX, int& nY, int  nZ)
{
	nX = nX + m_nLeft;
	nY = (nY + m_nTop + ((nZ * 887) >> 10)) * 2;
}
/*
bool Represent::SaveScreenToFile(const char* pszName)
{
...
bool Represent::SaveScreenToFile(const char* pszName, ScreenFileType eType, unsigned int nQuality)
{
	if (!pszName || !pszName[0])
		return 0;
