...
{
	if (pBuffer == NULL || nCount < 0 || g_pRepresent == NULL || nFontId <= 0)
		return 0;
	...
	while(nCurrentPos < nCount && nOutputLineCount < nMaxLineCount)
	{
		...
		g_pRepresent->OutputRichText(nFontId, pParam, pBuffer + nCurrentPos, nLen);
...
void SaveScreenToFile(ScreenFileType eType, unsigned int nQuality)
{
	...
	if(GetAPrintScreenFileName(&szFile[nLen], sizeof(szFile) - nLen))
	{
		...
		g_pRepresent->SaveScreenToFile(szFile, eType, nQuality);
	}
}
