...
void CSplash::OnPaint(HWND hwnd)
{
	...
	if(m_ShowText == 1)
	{
		HFONT hFont = CreateFont(10,0,0,0, FW_DONTCARE,FALSE,FALSE,FALSE,DEFAULT_CHARSET,OUT_OUTLINE_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, VARIABLE_PITCH | FF_DONTCARE, TEXT("VK Sans Serif"));
		SelectObject(hDC, hFont);
		SetBkMode(hDC,TRANSPARENT);
        //SetBkColor(hDC, RGB(255, 255, 255));
		//SetTextColor(hDC, m_TextColor);
		SetTextColor(hDC, RGB(0, 254, 255));
        if (m_SplashType == 2)
            TextOut(hDC, 45, 120, m_TextToDisplay, strlen(m_TextToDisplay));
        else
            TextOut(hDC, 45, 120, m_TextToDisplay, strlen(m_TextToDisplay));
	}
...
void CSplash::Start(int resourceId, HINSTANCE hins, COLORREF colTrans, bool hShowText, char hText[255], COLORREF hTextColor, int nType)
{
    m_SplashType = nType;
	m_ShowText = hShowText;
    //swprintf_s(m_TextToDisplay, 256, L"%s",hText);
...
void CSplash::Start(LPCTSTR lpszFileName, COLORREF colTrans, bool hShowText, char hText[255], COLORREF hTextColor, int nType)
{
    m_SplashType = nType;
    m_ShowText = hShowText;
    //swprintf_s(m_TextToDisplay, 256, L"%s",hText);
...
int CSplash::CloseSplash()
{
    ...
    while (1)
    {
        ...
        WinClasse = FindWindowExA(0, 0, "Sword3 Class", 0);
        GetWindowThreadProcessId(WinClasse, &lpdwProcessId);
        if (WinClasse)
        {
            if (m_hwnd)
            {
