#include "stdafx.h"
#include "Authenticate.h"
#include "Message.h"
#include "SplashScreen.h"
#include "Util.h"
#include "Protect.h"
#include <fstream>
using namespace std;

CAuthenticate gAuthenticate;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CAuthenticate::CAuthenticate() // OK
{
	this->m_socket = INVALID_SOCKET;

	this->m_event = WSA_INVALID_EVENT;

	this->m_AuthReceived = false;
}

CAuthenticate::~CAuthenticate() // OK
{
	this->Disconnect();
}

bool CAuthenticate::Init(void* function) // OK
{
	WSADATA wsa;

	if (WSAStartup(MAKEWORD(2, 2), &wsa) != 0)
	{
		return 0;
	}

	if ((this->m_socket = socket(PF_INET, SOCK_STREAM, 0)) == INVALID_SOCKET)
	{
		return 0;
	}

	if ((this->m_event = WSACreateEvent()) == WSA_INVALID_EVENT)
	{
		return 0;
	}

	this->wsProtocolCore = (void(*)(BYTE, BYTE*, int))function;

	return 1;
}

bool CAuthenticate::Connect(char* IpAddress, WORD port) // OK
{
	if (this->m_socket == INVALID_SOCKET)
	{
		return 0;
	}

	SOCKADDR_IN target;

	target.sin_family = AF_INET;
	target.sin_port = htons(port);
	target.sin_addr.s_addr = inet_addr(IpAddress);

	if (target.sin_addr.s_addr == INADDR_NONE)
	{
		HOSTENT* host = gethostbyname(IpAddress);

		if (host != 0)
		{
			memcpy(&target.sin_addr.s_addr, (*host->h_addr_list), host->h_length);
		}
	}

	if (connect(this->m_socket, (SOCKADDR*)&target, sizeof(target)) == SOCKET_ERROR)
	{
		if (WSAGetLastError() != WSAEWOULDBLOCK)
		{
			//gLog.Output(LOG_CONNECT,"[Connection] connect() failed with error: %d",WSAGetLastError());
			this->Disconnect();
			return 0;
		}
	}

	return 1;
}

void CAuthenticate::Disconnect() // OK
{
	if (this->m_socket != INVALID_SOCKET)
	{
		closesocket(this->m_socket);
		this->m_socket = INVALID_SOCKET;
	}
}

bool CAuthenticate::CheckState() // OK
{
	return ((this->m_socket == INVALID_SOCKET) ? 0 : 1);
}

bool CAuthenticate::DataRecv() // OK
{
	int count = 0, size = 0, result = 0;

	if ((result = recv(this->m_socket, (char*)&this->m_RecvBuff[this->m_RecvSize], (MAX_BUFF_SIZE - this->m_RecvSize), 0)) == SOCKET_ERROR)
	{
		if (WSAGetLastError() == WSAEWOULDBLOCK)
		{
			return 1;
		}
		else
		{
			//gLog.Output(LOG_CONNECT,"[Connection] recv() failed with error: %d",WSAGetLastError());
			this->Disconnect();
			return 0;
		}
	}

	this->m_RecvSize += result;

	if (this->m_RecvSize < 3)
	{
		return 1;
	}

	BYTE header, head;
	unsigned char decryptedtext[1024];
	memset(&decryptedtext, 0, sizeof(decryptedtext));

	while (true)
	{
		if (this->m_RecvBuff[count] == 0xC1)
		{
			header = this->m_RecvBuff[count];
			size = this->m_RecvBuff[count + 1];
			head = this->m_RecvBuff[count + 3];
		}
		else if (this->m_RecvBuff[count] == 0xC2)
		{
			header = this->m_RecvBuff[count];
			size = MAKEWORD(this->m_RecvBuff[count + 2], this->m_RecvBuff[count + 1]);
			head = this->m_RecvBuff[count + 4];
		}
		else
		{
			//gLog.Output(LOG_CONNECT,"[Connection] Protocol header error (header:%x)",this->m_RecvBuff[count]);
			this->Disconnect();
			return 0;
		}

		if (size <= 0 || size >= MAX_BUFF_SIZE)
		{
			//gLog.Output(LOG_CONNECT,"[Connection] Protocol size error (size:%d)",size);
			this->Disconnect();
			return 0;
		}

		if (size <= this->m_RecvSize)
		{
			if (header == 0xC1)
			{
				(*(BYTE*)&decryptedtext) = *(BYTE*)(&this->m_RecvBuff[count]);

				(BYTE)(decryptedtext[2]) = (BYTE)(this->m_RecvBuff[count + 2]);

				int decryptedtext_len;

				decryptedtext_len = DecryptStringProtect(&this->m_RecvBuff[count + 3], (size - 3), (unsigned char*)&decryptedtext[3]) + 3;

				decryptedtext[decryptedtext_len] = '\0';

				decryptedtext[1] = LOBYTE(decryptedtext_len);

				memmove(&this->m_RecvBuff[count], &decryptedtext, decryptedtext_len);

				PacketDecryptData(&this->m_RecvBuff[count + 3], (size - 3), this->m_RecvBuff[count + 2]);
				head = this->m_RecvBuff[count + 3];
			}
			else
			{
				(*(BYTE*)&decryptedtext) = *(BYTE*)(&this->m_RecvBuff[count]);

				(BYTE)(decryptedtext[3]) = (BYTE)(this->m_RecvBuff[count + 3]);

				int decryptedtext_len;

				decryptedtext_len = DecryptStringProtect(&this->m_RecvBuff[count + 4], (size - 4), (unsigned char*)&decryptedtext[4]) + 4;

				decryptedtext[decryptedtext_len] = '\0';

				decryptedtext[2] = LOBYTE(decryptedtext_len);

				memmove(&this->m_RecvBuff[count], &decryptedtext, decryptedtext_len);

				PacketDecryptData(&this->m_RecvBuff[count + 4], (size - 4), this->m_RecvBuff[count + 3]);

				head = this->m_RecvBuff[count + 4];

			}

			this->wsProtocolCore(head, &this->m_RecvBuff[count], size);

			count += size;
			this->m_RecvSize -= size;

			if (this->m_RecvSize <= 0)
			{
				break;
			}
		}
		else
		{
			if (count > 0 && this->m_RecvSize > 0 && this->m_RecvSize < MAX_BUFF_SIZE)
			{
				memmove(this->m_RecvBuff, &this->m_RecvBuff[count], this->m_RecvSize);
			}

			break;
		}
	}

	return 1;
}

bool CAuthenticate::DataSend(BYTE* lpMsg, int size) // OK
{
	this->m_critical.lock();

	if (this->m_socket == INVALID_SOCKET)
	{
		this->m_critical.unlock();
		return 0;
	}

	BYTE buff[MAX_BUFF_SIZE];

	memcpy(buff, lpMsg, size);

	unsigned char ciphertext[1024];

	memset(&ciphertext, 0, sizeof(ciphertext));

	if (buff[0] == 0xC1)
	{
		//PacketEncryptData(&buff[3], (size - 3), buff[2]);

		(*(BYTE*)&ciphertext) = (BYTE)(buff[0]);

		(BYTE)(ciphertext[2]) = (BYTE)(buff[2]);

		PacketEncryptData(&buff[3], (size - 3), buff[2]);

		size = EncryptStringProtect((unsigned char*)&buff[3], (size - 3), (unsigned char*)&ciphertext[3]) + 3;

		ciphertext[1] = LOBYTE(size);
	}
	else
	{
		//PacketEncryptData(&buff[4], (size - 4), buff[3]);

		(*(BYTE*)&ciphertext) = (BYTE)(buff[0]);

		(BYTE)(ciphertext[3]) = (BYTE)(buff[3]);

		PacketEncryptData(&buff[4], (size - 4), buff[3]);

		size = EncryptStringProtect((unsigned char*)&buff[4], (size - 4), (unsigned char*)&ciphertext[4]) + 4;

		ciphertext[2] = LOBYTE(size);

	}

	if (this->m_SendSize > 0)
	{
		if ((this->m_SendSize + size) >= MAX_BUFF_SIZE)
		{
			//gLog.Output(LOG_CONNECT,"[Connection] Max msg size (size:%d)",size);
			this->Disconnect();
			this->m_critical.unlock();
			return 0;
		}
		else
		{
			memcpy(&this->m_SendBuff[this->m_SendSize], ciphertext, size);
			this->m_SendSize += size;
			this->m_critical.unlock();
			return 1;
		}
	}

	int count = 0, result = 0;

	while (size > 0)
	{
		if ((result = send(this->m_socket, (char*)&ciphertext[count], size, 0)) == SOCKET_ERROR)
		{
			if (WSAGetLastError() == WSAEWOULDBLOCK)
			{
				if ((this->m_SendSize + size) >= MAX_BUFF_SIZE)
				{
					//gLog.Output(LOG_CONNECT,"[Connection] Max msg size (size:%d)",size);
					this->Disconnect();
					this->m_critical.unlock();
					return 0;
				}
				else
				{
					memcpy(&this->m_SendBuff[this->m_SendSize], &ciphertext[count], size);
					this->m_SendSize += size;
					this->m_critical.unlock();
					return 1;
				}
			}
			else
			{
				//gLog.Output(LOG_CONNECT,"[Connection] send() failed with error: %d",WSAGetLastError());
				this->Disconnect();
				this->m_critical.unlock();
				return 0;
			}
		}
		else
		{
			count += result;
			size -= result;
		}
	}

	this->m_critical.unlock();

	return 1;
}

bool CAuthenticate::DataSendEx() // OK
{
	this->m_critical.lock();

	int count = 0, result = 0;

	while (this->m_SendSize > 0)
	{
		if ((result = send(this->m_socket, (char*)&this->m_SendBuff[count], this->m_SendSize, 0)) == SOCKET_ERROR)
		{
			if (WSAGetLastError() == WSAEWOULDBLOCK)
			{
				memmove(this->m_SendBuff, &this->m_SendBuff[count], this->m_SendSize);
				this->m_critical.unlock();
				return 1;
			}
			else
			{
				//gLog.Output(LOG_CONNECT,"[Connection] send() failed with error: %d",WSAGetLastError());
				this->Disconnect();
				this->m_critical.unlock();
				return 0;
			}
		}
		else
		{
			count += result;
			this->m_SendSize -= result;
		}
	}

	this->m_critical.unlock();

	return 1;
}

DWORD WINAPI CAuthenticate::ClientWorkerThread(CAuthenticate* lpConnection) // OK
{
	while (true)
	{
		DWORD result = WSAWaitForMultipleEvents(1, &lpConnection->m_event, 1, WSA_INFINITE, 0);

		if ((result == WSA_WAIT_FAILED) || (result == WSA_WAIT_TIMEOUT))
		{
			continue;
		}

		WSANETWORKEVENTS NetworkEvents;

		if (WSAEnumNetworkEvents(lpConnection->m_socket, lpConnection->m_event, &NetworkEvents) == SOCKET_ERROR)
		{
			lpConnection->m_critical.lock();
			lpConnection->Disconnect();
			lpConnection->m_critical.unlock();
			return 0;
		}

		if ((NetworkEvents.lNetworkEvents & FD_READ) != 0)
		{
			if (NetworkEvents.iErrorCode[FD_READ_BIT] == 0)
			{
				gAuthenticate.DataRecv();
			}
			else
			{
				//gLog.Output(LOG_CONNECT,"[Connection] FD_READ failed with error: %d",NetworkEvents.iErrorCode[FD_READ_BIT]);
			}
		}

		if ((NetworkEvents.lNetworkEvents & FD_WRITE) != 0)
		{
			if (NetworkEvents.iErrorCode[FD_WRITE_BIT] == 0)
			{
				gAuthenticate.DataSendEx();
			}
			else
			{
				//gLog.Output(LOG_CONNECT,"[Connection] FD_WRITE failed with error: %d",NetworkEvents.iErrorCode[FD_WRITE_BIT]);
			}
		}

		if ((NetworkEvents.lNetworkEvents & FD_CLOSE) != 0)
		{
			if (NetworkEvents.iErrorCode[FD_CLOSE_BIT] == 0)
			{
				gAuthenticate.Disconnect();
			}
			else
			{
				//gLog.Output(LOG_CONNECT,"[Connection] FD_CLOSE failed with error: %d",NetworkEvents.iErrorCode[FD_CLOSE_BIT]);
			}
		}
	}

	return 0;
}

void CAuthenticate::Check()
{
	//char hid[36];

	//GetHardwareId(hid);

	/*if(strcmp(hid,gServerInfo.m_CustomerHardwareId) != 0)
	{
		AuthenticateError();
	}*/

	

	//int decryptedtext_len_1;
	//int decryptedtext_len_2;
	//int decryptedtext_len_3;

	//std::string buff_decodebase64_1 = base64_decode(this->Auth_Address_IP_1, false);
	//std::string buff_decodebase64_2 = base64_decode(this->Auth_Address_IP_2, false);
	//std::string buff_decodebase64_3 = base64_decode(this->Auth_Address_IP_3, false);

	/*ofstream myfile;
	myfile.open("E:\\decodebase64.txt");
	myfile << buff_decodebase64_1.c_str() << "\n";
	myfile << buff_decodebase64_2.c_str() << "\n";
	myfile << buff_decodebase64_3.c_str() << "\n";
	myfile.close();*/

	//unsigned char Decrypted_IPAddress_1[1024];
	//unsigned char Decrypted_IPAddress_2[1024];
	//unsigned char Decrypted_IPAddress_3[1024];

	//=======================================================

	//memset(&Decrypted_IPAddress_1, 0, sizeof(Decrypted_IPAddress_1));

	//decryptedtext_len_1 = DecryptStringProtect((unsigned char*)buff_decodebase64_1.c_str(), buff_decodebase64_1.size(), (unsigned char*)&Decrypted_IPAddress_1);

	//Decrypted_IPAddress_1[decryptedtext_len_1] = '\0';

	//=======================================================

	//memset(&Decrypted_IPAddress_2, 0, sizeof(Decrypted_IPAddress_2));

	//decryptedtext_len_2 = DecryptStringProtect((unsigned char*)buff_decodebase64_2.c_str(), buff_decodebase64_2.size(), (unsigned char*)&Decrypted_IPAddress_2);

	//Decrypted_IPAddress_2[decryptedtext_len_2] = '\0';

	//=======================================================

	//memset(&Decrypted_IPAddress_3, 0, sizeof(Decrypted_IPAddress_3));

	//decryptedtext_len_3 = DecryptStringProtect((unsigned char*)buff_decodebase64_3.c_str(), buff_decodebase64_3.size(), (unsigned char*)&Decrypted_IPAddress_3);

	//Decrypted_IPAddress_3[decryptedtext_len_3] = '\0';

	//=======================================================

	/*ofstream myfile2;
	myfile2.open("E:\\IPAddress.txt");
	myfile2 << Decrypted_IPAddress_1 << "\n";
	myfile2 << Decrypted_IPAddress_2 << "\n";
	myfile2 << Decrypted_IPAddress_3 << "\n";
	myfile2.close();*/

	//=======================================================

	if (gAuthenticate.Init(this->ProtocolCore) == 0)
	{
		//SplashScreen(&SplashError, 2, 1, gMessage.GetMessage(1), 5000);
		MessageBox(0, gMessage.GetMessage(1), "C1Game", MB_ICONINFORMATION);
		SafeExitProcess();
	}

	int nPortAuthen = gProtect.m_MainInfo.AuthServerPort;

	if (CLIENT_VERSION == 9)
	{
		nPortAuthen = 2023;
	}

	if (gAuthenticate.Connect((char*)gProtect.m_MainInfo.AuthIpAddress, nPortAuthen) == 0)
	{
		Sleep(1000);
		if (gAuthenticate.Connect((char*)gProtect.m_MainInfo.AuthServerSec, nPortAuthen) == 0)
		{
			Sleep(1000);
			//this->Disconnect();
			//MessageBox(0, gMessage.GetMessage(1), "C2Game", MB_ICONINFORMATION);
			//SafeExitProcess();
			//SplashScreen(&SplashError, 2, 1, gMessage.GetMessage(1), 5000);
			if (gAuthenticate.Connect((char*)gProtect.m_MainInfo.AuthIpAddress, nPortAuthen) == 0)
			{
				this->Disconnect();
				MessageBox(0, gMessage.GetMessage(1), "C2Game", MB_ICONINFORMATION);
				SafeExitProcess();
				//SplashScreen(&SplashError, 2, 1, gMessage.GetMessage(1), 5000);
			}
		}
	}
	//this->m_socket != INVALID_SOCKET
	if (WSAEventSelect(this->m_socket, this->m_event, FD_READ | FD_WRITE | FD_CLOSE) == SOCKET_ERROR)
	{
		this->Disconnect();
		//SplashScreen(&SplashError, 2, 1, gMessage.GetMessage(1), 5000);
		MessageBox(0, gMessage.GetMessage(1), "C3Game", MB_ICONINFORMATION);
		SafeExitProcess();
	}

	memset(this->m_RecvBuff, 0, sizeof(this->m_RecvBuff));

	this->m_RecvSize = 0;

	memset(this->m_SendBuff, 0, sizeof(this->m_SendBuff));

	this->m_SendSize = 0;

	CreateThread(0, 0, (LPTHREAD_START_ROUTINE)this->ClientWorkerThread, this, 0, 0);

	this->RequestInfoSend();

	//MessageBox(0, gMessage.GetMessage(1), "C4Game", MB_ICONINFORMATION);

	while (this->m_AuthReceived == false && this->m_socket != INVALID_SOCKET)
	{
		Sleep(500);
	}

	this->Disconnect();
}

void CAuthenticate::ProtocolCore(BYTE head, BYTE* lpMsg, int size) // OK
{
	switch (head)
	{
	case 0x00:
		gAuthenticate.AuthInfoRecv((PMSG_AUTH_INFO_RECV*)lpMsg);
		break;
	case 0x40:
		gAuthenticate.ConnectInfoRecv((SDHP_CONNECT_INFO_RECV*)lpMsg);
		break;
	}
}

void CAuthenticate::AuthInfoRecv(PMSG_AUTH_INFO_RECV* lpMsg) // OK
{
	if (lpMsg->result == 0)
	{
		if (lpMsg->status <= 0)
		{
			SplashScreen(&SplashError, 2, 1, gMessage.GetMessage(1), 5000);
		}
	}
	else
	{
		SplashScreen(&SplashError, 2, 1, gMessage.GetMessage(1), 5000);
	}

	this->m_AuthReceived = true;
}

void CAuthenticate::ConnectInfoRecv(SDHP_CONNECT_INFO_RECV* lpMsg) // OK
{
	/*ofstream myfile;
	myfile.open("c:\\example.txt");
	myfile << lpMsg->result << "\n";
	myfile << lpMsg->status << "\n";
	myfile << lpMsg->IpAddress << "\n";
	myfile << lpMsg->nPort << "\n";
	myfile.close();*/

	if (lpMsg->result == 0)
	{
		switch (lpMsg->status)
		{
		case 1:
			if (lpMsg->nSplashScreen == 1)
				SplashScreen(&SplashError, 2, 1, gMessage.GetMessage(2), 5000);
			else
				MessageBox(0, gMessage.GetMessage(2), "Game", MB_ICONINFORMATION);
			//SplashScreen(&SplashError, 2, 1, gMessage.GetMessage(2), 5000);
			break;
		default:
			if (lpMsg->nSplashScreen == 1)
				SplashScreen(&SplashError, 2, 1, gMessage.GetMessage(1), 5000);
			else
			{
				MessageBox(0, gMessage.GetMessage(1), "Game", MB_ICONINFORMATION);
				SafeExitProcess();
			}
			//SplashScreen(&SplashError, 2, 1, gMessage.GetMessage(1), 5000);
			break;
		}
	}
	else
	{
		//gReconnectStatus = 2;

		memcpy(&gIpAddress, &lpMsg->IpAddress, sizeof(gIpAddress));

		gProtect.m_MainInfo.ServerPort = lpMsg->nPort;

		this->m_SplashScreen = lpMsg->nSplashScreen;
		
		gProtect.m_MainInfo.nLimit = lpMsg->nLimit;

		this->m_AuthReceived = true;

	}
}

void CAuthenticate::RequestInfoSend() // OK
{
	SDHP_CONNECT_INFO_SEND pMsg;

	pMsg.header.set(0x40, sizeof(pMsg));

	strcpy_s(pMsg.LicenseId, gProtect.m_MainInfo.ClientSerial);

	pMsg.Version = gProtect.m_MainInfo.ClientVersion;

	this->DataSend((BYTE*)&pMsg, pMsg.header.size);
}

void CAuthenticate::SetAuthAddressIP(char* IpAddress, int numb)
{
	if (numb == 1)
	{
		memcpy(this->Auth_Address_IP_1, IpAddress, sizeof(this->Auth_Address_IP_1));
		this->Auth_Address_IP_1[sizeof(this->Auth_Address_IP_1)] = 0;
	}
	else if (numb == 2)
	{
		memcpy(this->Auth_Address_IP_2, IpAddress, sizeof(this->Auth_Address_IP_2));
		this->Auth_Address_IP_2[sizeof(this->Auth_Address_IP_2)] = 0;
	}
	else if (numb == 3)
	{
		memcpy(this->Auth_Address_IP_3, IpAddress, sizeof(this->Auth_Address_IP_3));
		this->Auth_Address_IP_3[sizeof(this->Auth_Address_IP_3)] = 0;
	}
	else
	{
		memcpy(this->Auth_Address_IP_1, IpAddress, sizeof(this->Auth_Address_IP_1));
		this->Auth_Address_IP_1[sizeof(this->Auth_Address_IP_1)] = 0;
	}
}