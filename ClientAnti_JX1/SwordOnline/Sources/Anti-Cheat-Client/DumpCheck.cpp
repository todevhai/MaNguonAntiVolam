...
bool CDumpCheck::CheckProcessName(wchar_t* sProcessName)
{
	for (std::vector<std::string>::iterator it = gListManager.gProcessListInfo.begin(); it != gListManager.gProcessListInfo.end(); it++)
	{
		if (memcmp(it->c_str(), sProcessName, sizeof(it->c_str())) == 0)
		{
			return 0;
		}
...
bool CDumpCheck::CheckWindowName(char* CaptionName)
{
	for (std::vector<std::string>::iterator it = gListManager.gWindowListStringInfo.begin(); it != gListManager.gWindowListStringInfo.end(); it++)
	{
		if (it->find(CaptionName) != std::string::npos)
		{
			return 0;
		}
...
bool CDumpCheck::Check(wchar_t* sTemp,char* CaptionName)
{
	...
	/*ofstream myfile;
	myfile.open("E:\\DumpList.txt");
	myfile << gListManager.gDumpListInfo.size() << "\n";
	myfile.close();*/
