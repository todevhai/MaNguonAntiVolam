...
void KMouseOver::Update(int nX, int nY)
{
	...
	{
		ALW_GetWndPosition(m_nLeft, m_nTop, m_nWndWidth, m_nWndHeight, false, false);
	}
...
{
	if (m_bShow == false || g_pRepresent == NULL)
		return;
	...
	if(m_nTitleLen > 0)
	{
		...
		while(true)
		{
			...
			g_pRepresent->OutputRichText(m_nFontSize, &Param, m_ObjTitle, m_nTitleLen, 0);
			Param.nSkipLine ++;
	...
	if(m_nDescLen > 0)
	{
		...
		Param.nX = m_nLeft + m_nIndent;
		Param.nY = Shadow.oPosition.nY;
		g_pRepresent->OutputRichText(m_nFontSize, &Param, m_ObjDesc, m_nDescLen, 0);
	}
