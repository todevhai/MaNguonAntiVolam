...
bool CD3D_Device::CreateDevice(D3DAdapterInfo* pAdapter,D3DDeviceInfo* pDevice,D3DModeInfo* pMode)
{
	...
	{
		assert(0);
		MessageBox(0, "[D3DRender]Warning: Direct3DCreate9Ex create failed.", "Info", MB_OK | MB_ICONINFORMATION);
		return hr;
	}

	hr = pD3D->CreateDeviceEx(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, g_hWnd,
		D3DCREATE_SOFTWARE_VERTEXPROCESSING,
		&d3dpp, NULL, &pDeviceEx);
	if (hr)
	{
		assert(0);
		MessageBox(0, "[D3DRender]Warning: Device create failed.", "Info", MB_OK | MB_ICONINFORMATION);
		return false;
	}

	/*HRESULT hResult = PDIRECT3D->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL,g_hWnd, D3DCREATE_SOFTWARE_VERTEXPROCESSING,&d3dpp,&m_pD3DDevice);
...
//		g_DebugLog("[D3DRender]D3DPTEXTURECAPS_NONPOW2CONDITIONAL is not supported");
	...
	if (m_pDevice->DeviceType == D3DDEVTYPE_REF)
		MessageBox(0, "[D3DRender]Warning: Couldnt' find any HAL devices, Using reference rasterizer", "Info", MB_OK | MB_ICONINFORMATION);
...
void CD3D_Device::SetupViewport(uint32 iLeft, uint32 iRight, uint32 iTop, uint32 iBottom, float fMinZ, float fMaxZ)
{
	...
	if (hResult != D3D_OK)
	{
		MessageBox(0, "[D3DRender]IDirect3DDevice::SetCurrentViewport failed.", "Info", MB_OK | MB_ICONINFORMATION);
		assert(0);
		return;
	}
}
