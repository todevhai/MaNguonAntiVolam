...
{
	return (new KRepresentShell2(m_Canvas, m_DirectDraw));
}
...
KRepresentShell2::KRepresentShell2(unsigned int* m_Canvas, unsigned int* m_DirectDraw)
{
...
bool KRepresentShell2::Create(int nWidth, int nHeight, bool bFullScreen)
{
	...
	if (m_DirectDraw.Init(m_CanvasLocal,m_DirectDrawLocal))
	{
		m_Canvas.Init(nWidth, nHeight, m_CanvasLocal, m_DirectDrawLocal);
		m_ImageStore.Init();
...
void KRepresentShell2::OutputText(int nFontId, const char* psText, int nCount, int nX, int nY, unsigned int Color, int nLineWidth, int nZ, unsigned int BorderColor)
{
	...
	if (i < RS2_MAX_FONT_ITEM_NUM && m_FontTable[i].pFontObj)
	{
		...
		//m_FontTable[i].pFontObj->SetBorderColor(0);
