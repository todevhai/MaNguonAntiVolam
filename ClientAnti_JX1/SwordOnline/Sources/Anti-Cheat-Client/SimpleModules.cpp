...
void CheckSystemInformation() // OK
{
	...
	gLog.Output(LOG_DEBUG,GetEncryptedString(0),Anti-Cheat-Client_VERSION,osvi.dwMajorVersion,osvi.dwMinorVersion,osvi.dwBuildNumber,(int)(((mstx.ullTotalPhys-mstx.ullAvailPhys)/1024)/1024),(int)((mstx.ullTotalPhys/1024)/1024));
}
...
bool API_INIT() // OK
{
	CProcessQuery::m_NtQuerySystemInformation = (NTQUERYSYSTEMINFORMATION)GetProcAddress(GetModuleHandle("Ntdll.dll"),"NtQueryInformationThread");

	API_Address_Table[API_Address_Count++] = (DWORD)GetProcAddress(GetModuleHandle("Kernel32.dll"),"CreateProcess");
	...
	//API_Address_Table[API_Address_Count++] = (DWORD)GetProcAddress(GetModuleHandle("Kernel32.dll"),"SuspendThread");
	//API_Address_Table[API_Address_Count++] = (DWORD)GetProcAddress(GetModuleHandle("Kernel32.dll"),"ResumeThread");
	//API_Address_Table[API_Address_Count++] = (DWORD)GetProcAddress(GetModuleHandle("Kernel32.dll"),"OpenThread");

	API_Address_Table[API_Address_Count++] = (DWORD)GetProcAddress(GetModuleHandle("Kernel32.dll"),"CreateRemoteThread");
	//API_Address_Table[API_Address_Count++] = (DWORD)GetProcAddress(GetModuleHandle("Kernel32.dll"),"WaitForSingleObject");
	//API_Address_Table[API_Address_Count++] = (DWORD)GetProcAddress(GetModuleHandle("Kernel32.dll"),"GetUserDefaultLCID");
	//API_Address_Table[API_Address_Count++] = (DWORD)GetProcAddress(GetModuleHandle("Kernel32.dll"),"CheckRemoteDebuggerPresent");
	//API_Address_Table[API_Address_Count++] = (DWORD)GetProcAddress(GetModuleHandle("Kernel32.dll"),"QueryFullProcessImageName");
	...
	API_Address_Table[API_Address_Count++] = CHECK_API_REDIRECT((DWORD)GetProcAddress(GetModuleHandle("Ntdll.dll"),"ZwReadVirtualMemory"));

	API_Address_Table[API_Address_Count++] = CHECK_API_REDIRECT((DWORD)GetProcAddress(GetModuleHandle("Ntdll.dll"),"RtlExitUserThread"));
...
bool MEMORY_PROTECTION_INIT() // OK
{
	...
	if((FileMappingHandle=CreateFileMapping(INVALID_HANDLE_VALUE,0,PAGE_READWRITE,0,sizeof(HACK_VERIFY_FILE_MAPPING),"data\\updatejx")) == 0)
	{
		return 0;
	}
