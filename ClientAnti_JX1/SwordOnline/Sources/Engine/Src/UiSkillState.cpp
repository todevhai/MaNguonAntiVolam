...
KUiSkillState*	KUiSkillState::m_pSelf = NULL;
KUiSkillStateData	KUiSkillState::ms_ListSkillsState[SKILLSTATE_SKILL_COUNT];

KUiSkillState::KUiSkillState()
{
	m_nNumSkills = 0;
	m_bLeft = true;
	m_nFont = 12;
...
//	功能：如果窗口正被显示，则返回实例指针
//--------------------------------------------------------------------------
KUiSkillState* KUiSkillState::GetIfVisible()
{
...
//--------------------------------------------------------------------------
KUiSkillState* KUiSkillState::OpenWindow(int bLeft)
{
	if (m_pSelf == NULL)	
	{
...
//--------------------------------------------------------------------------
void KUiSkillState::CloseWindow(bool bDestroy)
{
	if (m_pSelf)
	{
		m_pSelf->Hide();
...
//隐藏窗口
void KUiSkillState::Hide()
{
	Wnd_ReleaseCapture();
	KWndWindow::Hide();
}

void KUiSkillState::LoadConfig(KIniFile* pIni)
{
	/*memset(&ms_ListSkillsState, 0, sizeof(ms_ListSkillsState));
	if (pIni == NULL)
		return;
	...
	for (i = 0; i < SKILLSTATE_SKILL_COUNT; i++)
	{
		sprintf(szShortcutName, "ShortcutSkill_%d", i);
		pIni->GetStruct(SKILLTREE_SHORTCUT_SKILL_SAVE, szShortcutName,
			&ms_ListSkillsState[i], sizeof(KUiSkillStateData));

		if (ms_ListSkillsState[i].uGenre != CGOG_NOTHING)
		{
			ms_ListSkillsState[i].IS_LEFT_SKILL = !!ms_ListSkillsState[i].IS_LEFT_SKILL;
			for (j = 0; j < i; j++)
			{
				if (ms_ListSkillsState[i].uId == ms_ListSkillsState[j].uId &&
					ms_ListSkillsState[i].uGenre == ms_ListSkillsState[j].uGenre &&
					ms_ListSkillsState[i].IS_LEFT_SKILL == ms_ListSkillsState[j].IS_LEFT_SKILL)
				{
					ms_ListSkillsState[i].uGenre = CGOG_NOTHING;
					ms_ListSkillsState[i].uId = 0;
				}
			}
			bEncounter = true;
		}
	...
	do
	{
		...
		KUiSkillStateData	Skills[SKILLTREE_MAX_SKILL_COUNT];
		...
		if (bEncounter)
		{
			...
			{
				if (ms_ListSkillsState[i].uGenre == CGOG_NOTHING ||
					(!bLeft) != (!ms_ListSkillsState[i].IS_LEFT_SKILL))
					continue;
				for (j = 0; j < nNum; j++)
				{
					if (ms_ListSkillsState[i].uId == Skills[j].uId &&
						ms_ListSkillsState[i].uGenre == Skills[j].uGenre)
					{
						break;
					}
				}
...
//保存自定义配置
void KUiSkillState::SaveConfig(KIniFile* pIni)
{
	//if (pIni == NULL)
	//	return;
	//char szShortcutName[32];
	//for (int i = 0; i < SKILLSTATE_SKILL_COUNT; i++)
	//{
	//	sprintf(szShortcutName, "ShortcutSkill_%d", i);
	//	if (ms_ListSkillsState[i].uGenre != CGOG_NOTHING)
	//	{
...
void KUiSkillState::UpdateWndRect()
{
...
void KUiSkillState::UpdateData()
{
	...
	{
		if (ms_ListSkillsState[i].uGenre == CGOG_NOTHING ||
			(!m_bLeft) != (!ms_ListSkillsState[i].IS_LEFT_SKILL))
			continue;
		for (int j = 0; j < m_nNumSkills; j++)
		{
			if (ms_ListSkillsState[i].uId == m_Skills[j].uId &&
				ms_ListSkillsState[i].uGenre == m_Skills[j].uGenre)
			{
				break;
			}
			if (j >= m_nNumSkills)
...
int	KUiSkillState::PtInWindow(int x, int y)
{
...
int	KUiSkillState::GetSkillAtPos(int x, int y)
{
...
// 功能		: 载入界面方案
// -------------------------------------------------------------------------
void KUiSkillState::LoadScheme(const char* pScheme)
{
	char		Buff[128];
	KIniFile	Ini;
...
// -------------------------------------------------------------------------
// 函数		: KUiSkillState::WndProc
// 功能		: 窗口函数
// -------------------------------------------------------------------------
int KUiSkillState::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)
{
	int x, y, nIndex;
	int	nRet = 0;
	switch(uMsg)
			...
			if (nSkill >= 0)
			{
				if (g_MouseOver.IsMoseHoverWndObj(this, nSkill) == 0)
				{
					SetMouseHoverObjectDesc(this, nSkill, ms_ListSkillsState[nSkill].uGenre,
						ms_ListSkillsState[nSkill].uId, UOC_SKILL_TREE, x, y);
				}
			}
			else
...
void KUiSkillState::HandleShortcutKey(int nIndex)
{
	if (m_pSelf == NULL || nIndex < 0 || nIndex >= SKILLSTATE_SKILL_COUNT)
		return;
	if (GetIfVisible())
	{
		int	x, y;
		...
		ms_ListSkillsState[nIndex].uGenre = m_pSelf->m_Skills[nSkill].uGenre;
		ms_ListSkillsState[nIndex].uId = m_pSelf->m_Skills[nSkill].uId;
		ms_ListSkillsState[nIndex].IS_LEFT_SKILL = m_pSelf->m_bLeft;
		for (int i = 0; i < SKILLSTATE_SKILL_COUNT; i++)
		{
			if (i != nIndex)
			{
				if (ms_ListSkillsState[nIndex].uId == ms_ListSkillsState[i].uId &&
					ms_ListSkillsState[nIndex].IS_LEFT_SKILL == ms_ListSkillsState[i].IS_LEFT_SKILL &&
					ms_ListSkillsState[nIndex].uGenre == ms_ListSkillsState[i].uGenre)
				{
					ms_ListSkillsState[i].uGenre = CGOG_NOTHING;
					ms_ListSkillsState[i].uId = 0;
				}
	...
	else if (ms_ListSkillsState[nIndex].uGenre != CGOG_NOTHING)
	{
		g_pCoreShell->OperationRequest(GOI_SET_IMMDIA_SKILL,
			(unsigned int)&ms_ListSkillsState[nIndex],
			ms_ListSkillsState[nIndex].IS_LEFT_SKILL ? 0 : 1);
	}
}

void KUiSkillState::DirectHandleShortcutKey(int nIndex)
{
	if (nIndex < 0 || nIndex >= SKILLSTATE_SKILL_COUNT)
		return;
	...
	{
		g_pCoreShell->UseSkill(x, y, ms_ListSkillsState[nIndex].uId);
	}
}
...
// 功能	: 窗体绘制
// -------------------------------------------------------------------------
void KUiSkillState::PaintWindow()
{
...
//			else
		...
		/*for (int j = 0; j < SKILLSTATE_SKILL_COUNT; j++)
		{
			if (m_bLeft == (unsigned)ms_ListSkillsState[j].IS_LEFT_SKILL &&
				m_Skills[i].uGenre == ms_ListSkillsState[j].uGenre &&
				m_Skills[i].uId == ms_ListSkillsState[j].uId)
			{
				char szScript[64];
