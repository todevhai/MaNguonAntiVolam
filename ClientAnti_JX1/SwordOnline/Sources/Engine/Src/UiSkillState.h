...
#define	SKILLSTATE_SKILL_COUNT	10
...
struct KUiSkillStateData : public KUiGameObject
{
	union
	{
		int		nLevel;
...
class KUiSkillState : protected KWndImage
{
public:
	static KUiSkillState*	OpenWindow(int bLeft);			//打开窗口，返回唯一的一个类对象实例
	static KUiSkillState*	GetIfVisible();					//如果窗口正被显示，则返回实例指针
...
private:
	static KUiSkillState*	m_pSelf;
private:
	int				m_nNumSkills;
	KUiSkillStateData	m_Skills[SKILLTREE_MAX_SKILL_COUNT];
					//KUiSkillStateData::nLevel在此的含义表示位此技能显示在第几列
	...
	static KUiSkillStateData	ms_ListSkillsState[SKILLSTATE_SKILL_COUNT];
					//KUiSkillStateData::nLevel在此的含义0表示左手技能，1表示右手技能
