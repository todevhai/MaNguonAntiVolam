...
void KUiTongManager::Initialize()
{
	...
	AddChild(&m_BtnSaveMoney);
	AddChild(&m_BtnGetMoney);
	AddChild(&m_BtnSndMoney);
...
void KUiTongManager::LoadScheme(const char* pScheme)
{
	if(ms_pSelf)
	{
		...
		if(Ini.Load(Buff))
		{
	        ...
	        ms_pSelf->m_LienMinh.Init(&Ini, "TextLienMinh");
	        ms_pSelf->m_TongLevel.Init(&Ini, "TextTongLevel");
			...
			ms_pSelf->m_BtnSaveMoney.Init(&Ini, "BtnDepositMoney");
			ms_pSelf->m_BtnGetMoney.Init(&Ini, "BtnTakeMoney"); 
			ms_pSelf->m_BtnSndMoney.Init(&Ini, "BtnSalary");
...
int KUiTongManager::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)
{
	...
	case WND_N_BUTTON_CLICK:
		...
		else if (uParam == (unsigned int)&m_BtnSaveMoney)
		{
			m_nMoney = g_pCoreShell->GetGameData(GDI_PLAYER_HOLD_MONEY, 0, 0);
			KUiGetMoney::OpenWindow(0, m_nMoney,this, RESULT_T_SAVE,NULL);
		}
		else if (uParam == (unsigned int)&m_BtnGetMoney)
		{
//			KUiPopupPasswordQuery::OpenWindow(this,RESULT_T_GET);
			KUiGetMoney::OpenWindow(0,m_TongData.nMoney,this,RESULT_T_GET,NULL);
		}
		else if (uParam == (unsigned int)&m_BtnSndMoney)
		{
			KUiGetMoney::OpenWindow(0,m_TongData.nMoney,this,RESULT_T_SND,NULL);
		}
		break;
...
void KUiTongManager::UpdateTongInfoView()
{
	...
	m_MemberCount.SetText(itoa(m_TongData.nMemberCount + m_TongData.nManagerCount + m_TongData.nDirectorCount + 1, Buff, 10));
	m_Money.SetMoneyText(m_TongData.nMoney);
}
...
//		m_ListScroll.SetValueRange(0,m_List.GetCount() + pData->GetCount());
...
{
	m_MemberCount.SetText("Ch­a cã");
	m_MasterName.SetText("Ch­a cã");
	m_Alignment.SetText("Ch­a cã");
	m_TongName.SetText("Ch­a cã");
	m_Money.SetText("Ch­a cã");
	m_LienMinh.SetText("Kh«ng liªn minh");
	m_TongLevel.SetText("Ch­a cã");
	m_List.ResetContent();
...
void KUiTongManager::PopupConfirmWindow(const char* pszInfo, unsigned int uHandleID)
{
	...
	if(uHandleID == RESULT_T_LEAVE)
	    KUiTongGetString::OpenWindow(pszInfo, "100Íò", this, uHandleID, 1, 31);
	else if (pPlayer)
...
void KUiTongManager::UpdataMoney( int nMoney )
{
	m_TongData.nMoney = nMoney;
