...
void KWndText::SetMoneyText(int nNumber, char Separator)
{
	...
	{
		if (nNumber/10000 == 0)
			nLen = sprintf(Buff, "%d lng",nNumber%10000);
		else
		{
			if (nNumber%10000 == 0)
				nLen = sprintf(Buff, "%d v筺 lng",nNumber/10000);
			else
				nLen = sprintf(Buff, "%d v筺 %d lng",nNumber/10000,nNumber%10000);    
		}
		SetText(Buff, nLen);
	}
}  

void KWndText::SetNumberItem(int nNumber, char Separator)
{
	char    Buff[64];
	int        nLen;
	if (m_pText)
			nLen = sprintf(Buff, "%d c竔",nNumber);  
		SetText(Buff, nLen);
}  

void KWndText::SetXuText(int nNumber, char Separator)
    {
        char    Buff[64];
        int        nLen;
        if (m_pText)
        {
            ...
            {
                if (nNumber%10000 == 0)
                    nLen = sprintf(Buff, "%d v筺 xu",nNumber/10000);
                else
                    nLen = sprintf(Buff, "%d v筺 %d xu",nNumber/10000,nNumber%10000);    
            }
            SetText(Buff, nLen);
        }
    }
...
//--------------------------------------------------------------------------
// hien thi xep hang
void KWndText::Set5IntText(int nNumber, char Separator)
{
	char	Buff[5];
	int		nLen;
	if (m_pText)
...
//-----------hien thi toc do danh
void KWndText::SetAttackText(int nNumber1, int nNumber2, char Separator)
{
	char	Buff[15];
	int		nLen;
	if (m_pText)
	...
	if (m_pText)
	{
		...
		else
		{
			...
			nLen = sprintf(Buff, "Ch璦 tr飊g sinh");
				...
				nLen = sprintf(Buff, "Tr飊g sinh %d l莕", nNumber);
