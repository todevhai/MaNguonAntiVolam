...
#define SCHEME_INI_ITEM	"MoRongRuongHai.ini"

KUiExBox2* KUiExBox2::m_pSelf = NULL;
...
//--------------------------------------------------------------------------
KUiExBox2* KUiExBox2::GetIfVisible()
{
...
//--------------------------------------------------------------------------
KUiExBox2* KUiExBox2::OpenWindow()
{
	if (m_pSelf == NULL)
	{
		m_pSelf = new KUiExBox2;
		if (m_pSelf)
			m_pSelf->Initialize();
	}
	if (m_pSelf)
...
// π¶ƒ‹	: ≥ı ºªØ
// -------------------------------------------------------------------------
void KUiExBox2::Initialize()
{
	AddChild(&m_SelectionTwo);
	AddChild(&m_BuyOne);
	AddChild(&m_BuyThree);
	AddChild(&m_CloseBtn);
	AddChild(&m_ItemBoxHai);
	AddChild(&m_InfoTextPage);
	AddChild(&m_TextPage);

	

	m_ItemBoxHai.SetContainerId((int)UOC_EX_BOX2);
	char Scheme[256];
...
void KUiExBox2::UpdateData()
{
	...
	int nCount = g_pCoreShell->GetGameData(GDI_ITEM_IN_EX_BOX2, 0, 0);
	if (nCount == 0)
		return;

	if (pObjs = (KUiObjAtRegion*)malloc(sizeof(KUiObjAtRegion) * nCount))
	{
		g_pCoreShell->GetGameData(GDI_ITEM_IN_EX_BOX2, (unsigned int)pObjs, nCount);//µ•œﬂ≥Ã÷¥––£¨nCount÷µ≤ª±‰
...
// π¶ƒ‹	: ŒÔ∆∑±‰ªØ∏¸–¬
// -------------------------------------------------------------------------
void KUiExBox2::UpdateItem(KUiObjAtRegion* pItem, int bAdd)
{
	if (pItem)
	{
		...
		if (pItem->Obj.uGenre != CGOG_MONEY)
		{
			...
			if (bAdd)
				m_ItemBoxHai.AddObject(&Obj, 1);
			else
				m_ItemBoxHai.RemoveObject(&Obj);
		}
		/*else
		{
...
// π¶ƒ‹	: ‘ÿ»ÎΩÁ√Ê∑Ω∞∏
// -------------------------------------------------------------------------
void KUiExBox2::LoadScheme(const char* pScheme)
{
	char		Buff[128];
	KIniFile	Ini;
	...
	if (m_pSelf && Ini.Load(Buff))
	{
		...
		m_pSelf->m_SelectionTwo.Init(&Ini, "SelectionOne");
		m_pSelf->m_BuyOne.Init(&Ini, "BuyOne");
		m_pSelf->m_BuyThree.Init(&Ini, "BuyThree");
		m_pSelf->m_CloseBtn.Init(&Ini, "CloseBtn");
		m_pSelf->m_ItemBoxHai.Init(&Ini, "ItemBoxHai");
		m_pSelf->m_ItemBoxHai.EnableTracePutPos(true);
		m_pSelf->m_InfoTextPage.Init(&Ini, "InfoTextPage");
		m_pSelf->m_InfoTextPage.SetText("R≠¨ng 2");
...
// π¶ƒ‹	: ¥∞ø⁄∫Ø ˝
// -------------------------------------------------------------------------
int KUiExBox2::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)
{
	switch(uMsg)
	{
...
void KUiExBox2::OnItemPickDrop(ITEM_PICKDROP_PLACE* pPickPos, ITEM_PICKDROP_PLACE* pDropPos)
{
