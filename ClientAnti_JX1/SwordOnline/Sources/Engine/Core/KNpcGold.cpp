...
{
	if (/*!m_nIsGold || */!m_nIsGolding || m_nGoldType >= 10)
		return;
...
//	功能：按一定概率变成黄金怪物
//---------------------------------------------------------------------------
void	KNpcGold::RandChangeGold(int nRand,BOOL bGold)
{
	if (/*m_nIsGold || */this->m_nIsGolding)
		return;
	if (g_Random(100) >= nRand)
		return;
	if (NpcSet.m_cGoldTemplate.m_nEffectTypeNum <= 0)
		return;


	if (g_Random(100) >= 60 && !bGold)
		m_nGoldType = GetRandomNumber(0,3);
	else if (!bGold)
		m_nGoldType = GetRandomNumber(3,5);
	else
		m_nGoldType = 10;
// 	else 	if (g_Random(100) >= 30)
// 		m_nGoldType = GetRandomNumber(11,15);
// 	else	if (g_Random(100) >= 30)
// 		m_nGoldType = GetRandomNumber(16,20);
// 	else	if (g_Random(100) >= 30)
// 		m_nGoldType = GetRandomNumber(21,25);
// 	else	if (g_Random(100) >= 50)
// 		m_nGoldType = GetRandomNumber(26,30);
// 	else
// 		m_nGoldType = GetRandomNumber(31,35);
	m_nIsGolding = 1;
	if (!bGold)
	{
	KNpcGoldTemplateInfo	*pInfo = &NpcSet.m_cGoldTemplate.m_sInfo[m_nGoldType];
...
void	KNpcGold::SetGoldCurrentType(int nType)
{
	...
	sSync.ProtocolType = s2c_npcgoldchange;
	sSync.m_dwNpcID = Npc[this->m_nNpcIdx].m_dwID;
	sSync.m_wGoldFlag = m_nGoldType + 1;
	Npc[m_nNpcIdx].SendDataToNearRegion((LPVOID)&sSync, sizeof(NPC_GOLD_CHANGE_SYNC));
	#endif
}
