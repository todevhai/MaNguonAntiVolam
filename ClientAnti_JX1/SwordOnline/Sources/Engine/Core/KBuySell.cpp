...
//	g_SetFilePath("\\");
	...
	for (int k = 0; k < nHeight; k++)
	{
		...
		int nGolden = 0,nJiFen = 0;
		GoodsFile.GetInteger(k + 2, 7, 0, &nGolden);
		GoodsFile.GetInteger(k + 2, 8, 0, &nJiFen);
		...
		case item_medicine:
			...
			break;
		case item_mine:
			ItemGen.Gen_Mine(ItemGenerator.nParticularType, &m_Item[k], 1, ItemGenerator.nLevel, ItemGenerator.nSeriesReq,ItemGenerator.nDetailType);
			break;
...
BOOL KBuySell::Buy(int nPlayerIdx, int nBuy, int nBuyIdx, int nPlace, int nX, int nY)
{
	...
	if (Player[nPlayerIdx].m_ItemList.GetEquipmentXu() < m_Item[nIdx].GetPriceXu())
		return FALSE;
	...
	if (m_Item[nIdx].GetGenre() == item_task)
	{
	Player[nPlayerIdx].PayXu(m_Item[nIdx].GetPriceXu());
...
void KBuySell::OpenMarket(int nShop)
{
	if (nShop < 0 || nShop >= m_Height)
		return;
	Player[CLIENT_PLAYER_INDEX].m_BuyInfo.m_nBuyIdx = nShop;
...
void KBuySell::OpenMarket(int nPlayerIdx, int nShop)
{
	if (nPlayerIdx <= 0 || nPlayerIdx > MAX_PLAYER)
	{
		return;
	...
	SALE_MARKET_SYNC marketSync;
	marketSync.ProtocolType = s2c_openmarketbox;
	marketSync.nShopID = nShop;
	g_pServer->PackDataToClient(Player[nPlayerIdx].m_nNetConnectIdx, &marketSync, sizeof(SALE_MARKET_SYNC));
}
#endif
