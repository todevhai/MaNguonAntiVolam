	...
	case	MISSLE_MMK_Helix:							//	阿基米德螺旋线（暗黑二游侠的Bless Hammer）
		{
			...
			if (m_nTempParam1 == 0)
			{
// 				if ((Npc[m_nLauncher].m_Dir > 48 &&  Npc[m_nLauncher].m_Dir <= 64 ) 
// 					||
// 					 (Npc[m_nLauncher].m_Dir >=  0 && Npc[m_nLauncher].m_Dir <16))
// 				{
// 					m_nParam1 = 0;
// 				} 
// 				else if (Npc[m_nLauncher].m_Dir > 16 && Npc[m_nLauncher].m_Dir <= 48 )
// 				{
// 					m_nParam1 = 0;
// 				}
				m_nParam1 = 0;
...
BOOL KMissle::ProcessDamage(int nNpcId)
{
	...
	if (m_pMagicAttribsData) 
	{
		if (Npc[nNpcId].ReceiveDamage(m_nLauncher, m_bIsMelee, m_pMagicAttribsData->m_pDamageMagicAttribs, m_bUseAttackRating, m_bDoHurt,m_nEnChance))
		{
