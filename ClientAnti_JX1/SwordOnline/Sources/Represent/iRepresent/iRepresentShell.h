...
public:
	...
	extern "C" __declspec(dllimport) iRepresentShell* CreateRepresentDirect(unsigned int* m_Canvas, unsigned int* m_DirectDraw);
	//=========测试模块性能，判断是否推荐使用===============
	//可选存在的接口函数
...
//------模块的接口函数的原型的定义------
typedef iRepresentShell*					(*fnCreateRepresentDirect)(unsigned int* m_Canvas, unsigned int* m_DirectDraw);
//typedef bool								(*fnRepresentIsModuleRecommended)(unsigned int* m_Canvas, unsigned int* m_DirectDraw);
//------接口函数的函数名字符串----------
#define	FN_CREATE_REPRESENT_SHELL			"CreateRepresentDirect"
