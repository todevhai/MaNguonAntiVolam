...
bool pack(const char *dir, const char *pack_name = 0) {
	...
	addDirectory(output, dir);					//压缩指定目录
	if(!bCheck) {
		memset(&header, 0, sizeof(header));
...
/*	pack("d:\\resource\\font");
	...
	pack("d:\\resource\\游戏资源", "d:\\resource\\resource.pak");*/
/*	ZCache cache(65536 * 1024);
	ZPackFile pack("d:\\resource\\font.pak", &cache);
	char *data = pack.getData("\\gbk_fs12.fnt");*/
	return 0;
}