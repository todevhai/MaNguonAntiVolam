...
def tao_tep(rel, noi_dung, why):
    """Tao mot tep MOI trong cay nguon (idempotent).

    Chi dung cho HEADER: them mot .h khong phai sua .vcxproj, con them .cpp
    thi phai - va sua .vcxproj bang va byte la duong hong CI da biet.
    """
    global n_ok, n_skip, n_hong
    if why in DISABLED:
        print('  [TAT] %-42s %s' % (rel, why)); return
    p = os.path.join(SRC, rel)
    if os.path.exists(p) and open(p, 'rb').read() == noi_dung:
        print('  bo qua (da co): %s' % rel); n_skip += 1; return
    os.makedirs(os.path.dirname(p), exist_ok=True)
    open(p, 'wb').write(noi_dung)
    print('  tao: %-42s %s' % (rel, why)); n_ok += 1
...
edit('Core/Src/KPlayer.cpp',
     ...
     b'\t\tif (m_nPeapleIdx)\r\n'
     b'\t\t{\r\n'
     b'\t\t\t/* CHI phat khi da TRONG TAM. Ngoai tam thi giu duong cu (di toi\r\n'
     b'\t\t\t   roi danh): phat tu xa thi client tien len con may chu giu vi tri\r\n'
     b'\t\t\t   cu roi keo ve - nhin ra ngoai la nhan vat giat lui tung nac\r\n'
     b'\t\t\t   giong luc tim duong bi ket. */\r\n'
     b'\t\t\tISkill* pChieuTrai = g_SkillManager.GetSkill(m_nLeftSkillID, 1);\r\n'
     b'\t\t\tint nTamDanh = pChieuTrai ? pChieuTrai->GetAttackRadius() : 0;\r\n'
     b'\t\t\tif (nTamDanh > 0 && NpcSet.GetDistance(m_nIndex, m_nPeapleIdx) <= nTamDanh)\r\n'
...
edit('Represent/Represent2/KRepresentShell2.cpp',
     ...
     b'\t\t\t\t\t\t\tbreak;\r\n',
     b'\t\t\t\t\t\t\t/* CHINH TEP SPR KHAI CACH VE: byte 0x16 cua SPRHEAD\r\n'
     b'\t\t\t\t\t\t\t   (Reserved[1]) bang 1 thi tron SCREEN, khong thi alpha\r\n'
     b'\t\t\t\t\t\t\t   thuong. Do ma may represent2.dll ban hoan thien\r\n'
     b'\t\t\t\t\t\t\t   12/09/2026: 0x100049ca so byte do roi EP kieu ve sang so 7\r\n'
     b'\t\t\t\t\t\t\t   (KCanvas::DrawSpriteScreen), khong thi di bang nhay thuong.\r\n'
     b'\t\t\t\t\t\t\t   Quet du lieu: sprite doi 8.x (thu muc 150/, 1502/) khai 1,\r\n'
     b'\t\t\t\t\t\t\t   sprite doi 2003 khai 0. Ve nham chieu nao cung hong:\r\n'
     b'\t\t\t\t\t\t\t   alpha len sprite 8.x ra QUANG DEN, screen len sprite 2003\r\n'
     b'\t\t\t\t\t\t\t   ra KHOI MAU DAC. */\r\n'
     b'\t\t\t\t\t\t\tif (pSprHeader->Reserved[1] == 1)\r\n'
     b'\t\t\t\t\t\t\t\tm_Canvas.DrawSpriteAdd(nX, nY, pFrame->Width, pFrame->Height,\r\n'
     b'\t\t\t\t\t\t\t\t\tpFrame->Sprite, pPalette, 1);\r\n'
     b'\t\t\t\t\t\t\telse\r\n'
     b'\t\t\t\t\t\t\t\tm_Canvas.DrawSpriteAlpha(nX, nY, pFrame->Width, pFrame->Height,\r\n'
     b'\t\t\t\t\t\t\t\t\tpFrame->Sprite, pPalette, pTemp->Color.Color_b.a / 8);\r\n'
     b'\t\t\t\t\t\t\tbreak;\r\n',
...
# Phep tron nam RIENG mot header de jxstudio dich lai duoc tren macOS ma
# khong phai cat than ham theo moc. Xem
# docs/08-chien-dau-va-npc/21-goi-engine-tu-jxstudio.md
tao_tep('Engine/Src/KTronSprite.h',
     b'//---------------------------------------------------------------------------\r\n'
     b'// Phep tron sprite CONG SANG - TACH RIENG de dung chung.\r\n'
     b'//\r\n'
     b'// Vi sao tach: jxstudio (macOS) dich CHINH tep nay thanh .dylib de xem sprite\r\n'
     b'// hieu ung dung y nhu trong game. Truoc 13/09/2026 no cat than ham theo moc\r\n'
     b'// trong ban va - moc truot la hong am tham. Nay ca hai ben cung MOT tep.\r\n'
     b'//\r\n'
     b'// KHONG duoc dinh KCanvas/KDrawNode/DirectDraw o day: tep nay phai dich duoc\r\n'
     b'// bang clang tren macOS. Cat va khoa mat ve van nam ben g_DrawSpriteAdd.\r\n'
     b'//\r\n'
     ...
     b'//---------------------------------------------------------------------------\r\n'
     b'#ifndef KTronSprite_H\r\n'
     b'#define KTronSprite_H\r\n'
     b'inline void g_TronSpriteVaoDem(\r\n'
     b'	char* pDongDau, int nPitch, int bLa565,\r\n'
     b'	const unsigned char* pNguon, const unsigned short* pBangMau,\r\n'
     b'	int nRong, int nCao,\r\n'
     b'	int nHangDau, int nHangCuoi, int nCotDau, int nCotCuoi, int nLechX)\r\n'
     b'{\r\n'
     b'	/* 0x07e0f81f la mat na cua 565, con lai la 555 */\r\n'
     b'	int nDichR = bLa565 ? 11 : 10;\r\n'
     b'	int nDichG = 5;\r\n'
     b'	int nTranR = 31;\r\n'
     b'	int nTranG = bLa565 ? 63 : 31;\r\n'
     b'	int nTranB = 31;\r\n'
     b'	int nChiaG = bLa565 ? 6 : 5;\r\n'
     ...
     b'				*pO = (unsigned short)((nR << nDichR) | (nG << nDichG) | nB);\r\n'
     b'			}\r\n'
     b'			nCot++;\r\n'
     b'			if (nCot >= nRong)\r\n'
     b'			{\r\n'
     ...
     b'}\r\n'
     b'#endif\r\n',
     'tach phep tron sprite ra header dung chung')
...
edit('Engine/Src/KDrawBase.cpp',
     ...
     b'\r\n'
     b'\r\n'
     b'	g_TronSpriteVaoDem(\r\n'
     b'		(char*)pBuffer + Clipper.y * nPitch, nPitch,\r\n'
     b'		pCanvas->m_nMask32 == 0x07e0f81f,\r\n'
     b'		(const unsigned char*)pNode->m_pBitmap,\r\n'
     b'		(const unsigned short*)pNode->m_pPalette,\r\n'
     b'		pNode->m_nWidth, pNode->m_nHeight,\r\n'
     b'		Clipper.top, Clipper.top + Clipper.height,\r\n'
     b'		Clipper.left, pNode->m_nWidth - Clipper.right, Clipper.x);\r\n'
...
edit('S3Client/Ui/UiCase/UiMiniMap.cpp',
     ...
     _crlf(b'\t\t\tg_pRepresentShell->DrawPrimitives(1, &rect, RU_T_RECT, true);\n'
           b'\n'
           b'\t\t\t/* Bo cuoc giua duong (ket, hoac bam trung tuong/nha nen dich khong\n'
           b'\t\t\t   dung duoc): nhan vat dung han ma chua toi noi -> tat vector, dung\n'
           b'\t\t\t   de no treo mai. Dem vai khung cho chac, tranh tat nham o khe giua\n'
           b'\t\t\t   hai buoc di. */\n'
           b'\t\t\tif (g_nDichSpaceX >= 0)\n'
           b'\t\t\t{\n'
           b'\t\t\t\tstatic int s_nDungYen = 0;\n'
           b'\t\t\t\tif (g_pCoreShell->GetGameData(GDI_PLAYER_GOING_TO_DEST, 0, 0))\n'
           b'\t\t\t\t\ts_nDungYen = 0;\n'
           ...
           b'\t\t\t\t\t\t\t/* Duong bat dau tu NGUOI CHOI, khong phai tam ban do.\n'
           b'\t\t\t\t\t\t\t   Khi keo ban do, tam ban do = tam da keo (nOrigFocus+nFocusOffset),\n'
           b'\t\t\t\t\t\t\t   con nguoi choi o nOrigFocus -> pixel nguoi choi = tam - offset/scale. */\n'
           b'\t\t\t\t\t\t\tline.oPosition.nX = nTamX - MapInfo.nFocusOffsetH / MapInfo.nScallH;\n'
           b'\t\t\t\t\t\t\tline.oPosition.nY = nTamY - MapInfo.nFocusOffsetV / MapInfo.nScallV;\n'
...
edit('Core/Src/KItem.cpp',
     ...
     _crlf(b'#include "KMagicDesc.h"\n'
           ...
           b'}\n'
           b'\n'
           b'/* Hoang Kim: each magic slot has its own range in magicattrib_ge.txt (goldequip columns 47..52 give the\n'
           b'   1-based row). The blue magicattrib.txt range does not apply - gold values go past it. */\n'
           b'static BOOL GoldMagicRange(int nGoldId, int nSlot, int* pnMin, int* pnMax)\n'
           b'{\n'
           b'\tconst KLibOfBPT* pLib = ItemGen.GetBPTLib();\n'
           b'\tif (nGoldId <= 0 || nGoldId > pLib->GetGoldItemNumber() || nSlot < 0 || nSlot >= 6)\n'
           b'\t\treturn FALSE;\n'
           b'\tconst KBASICPROP_EQUIPMENT_GOLD* pRec = pLib->GetGoldItemRecord(nGoldId - 1);\n'
           b'\tif (!pRec)\n'
           b'\t\treturn FALSE;\n'
           b'\tint nIdx = pRec->m_aryMagicIdx[nSlot];\n'
           b'\tif (nIdx <= 0 || nIdx > pLib->GetGoldMagicNumber())\n'
           b'\t\treturn FALSE;\n'
           b'\tconst KBASICPROP_GOLDMAGIC* pMA = pLib->GetGoldMagicRecord(nIdx - 1);\n'
           b'\tif (!pMA || pMA->m_aryRange[0].nMax <= pMA->m_aryRange[0].nMin)\n'
           b'\t\treturn FALSE;\t/* fixed value, nothing to show */\n'
           b'\t*pnMin = pMA->m_aryRange[0].nMin;\n'
           b'\t*pnMax = pMA->m_aryRange[0].nMax;\n'
           b'\treturn TRUE;\n'
           b'}\n'
           b'#endif'),
...
edit('Core/Src/KItem.cpp',
     ...
     _crlf(b'\t\t\t\t\tstrcat(pszMsg, "<color=DBlue>");\n\t\t\t}\n\t\t}\n\t\tstrcat(pszMsg, pszInfo);\n'
           b'\t\t{\n'
           b'\t\t\tint nMin, nMax;\n'
           b'\t\t\tBOOL bCoKhoang = (m_CommonAttrib.nGoldId > 0)\n'
           b'\t\t\t\t? GoldMagicRange(m_CommonAttrib.nGoldId, i, &nMin, &nMax)\n'
           b'\t\t\t\t: MagicAttribRange(m_aryMagicAttrib[i].nAttribType, &nMin, &nMax);\n'
           b'\t\t\tif (strlen(pszMsg) < 12000 && bCoKhoang)\n'
           b'\t\t\t{\n'
           b'\t\t\t\tchar szKhoang[48];\n'
...
edit('Core/Src/CoreShell.cpp',
     ...
     _crlf(b'\t\tg_ScenePlace.ViewPortCoordToSpaceCoord(nX, nY, nZ);\n'
           ...
           b'\t\t\tAutoPathClearStuck();\t// lenh di MOI -> xoa blacklist o ket cu\n'
           b'\t\t\tint bApFin = 1;\n'
...
edit('Core/Src/KNpc.cpp',
     ...
     _crlf(b'\tBOOL bApStuck = FALSE;\n'
           ...
           b'\t\t// KET = KHONG gan them des sau nhieu khung LIEN TUC. Chi reset dem khi\n'
           b'\t\t// that su TIEN GAN des (nApDist < ky luc - 4). Phep cu "cua so 30 khung"\n'
           b'\t\t// bi qua khi nhan vat rung lac quanh khe hep (moi 30 khung lai nhich\n'
           b'\t\t// mot ti -> reset -> ket vo han khong bao gio bao). Nay dung ky luc min:\n'
           b'\t\t// rung lac khong gan them -> dem tang deu -> 45 khung (~1.5s) bao ket.\n'
           b'\t\tif (nApDist + 4 < m_nAutoPathLastDist)\n'
           b'\t\t{\n'
           b'\t\t\tm_nAutoPathLastDist = nApDist;\t// ky luc moi -> con tien\n'
           b'\t\t\tm_nAutoPathNoProg = 0;\n'
           b'\t\t}\n'
           b'\t\telse if (++m_nAutoPathNoProg >= 45)\n'
           b'\t\t{\n'
           b'\t\t\tbApStuck = TRUE;\t// 45 khung khong gan them des -> ket that (khe hep)\n'
           b'\t\t}\n'
           b'\t}\n'
           ...
           b'\t\t\t// tinh lai -> A* vong qua thay vi lao vao lai. Chi khi bApStuck de\n'
           b'\t\t\t// khong blacklist nham o tot luc den dich binh thuong (nRet==0).\n'
           b'\t\t\tif (bApStuck)\n'
           b'\t\t\t{\n'
           b'\t\t\t\tint nSdx = m_DesX - nApCurMx, nSdy = m_DesY - nApCurMy;\n'
           b'\t\t\t\tint nSad = (nSdx < 0 ? -nSdx : nSdx) + (nSdy < 0 ? -nSdy : nSdy);\n'
           b'\t\t\t\tif (nSad > 0)\n'
           b'\t\t\t\t\tAutoPathMarkStuckMps(nApCurMx + nSdx * 48 / nSad, nApCurMy + nSdy * 48 / nSad);\n'
           b'\t\t\t}\n'
           ...
           b'\t\t\t\tint nApWp = AutoPathFind(nApCurMx, nApCurMy, nApGx, nApGy, m_AutoPathX, m_AutoPathY, AUTOPATH_MAX_WP);\n'
           b'\t\t\t\tif (nApWp > 0)\n'
           b'\t\t\t\t{\n'
           b'\t\t\t\t\tm_nAutoPathCnt = nApWp;\n'
           b'\t\t\t\t\tm_nAutoPathIdx = 0;\n'
           b'\t\t\t\t\tm_nAutoPathRecalc++;\n'
...
edit('Core/Src/KNpc.cpp',
     ...
     _crlf(b'\tProcCommand(m_ProcessAI);\n'
           ...
           b'\t// nem than vao goc nha. Chay ca khi dang di. Vi tri that do server sync.\n'
           b'\tif (IsPlayer() && m_nAutoPathCnt > 0 && (m_LoopFrames & 3) == 0)\n'
           b'\t{\n'
           b'\t\tint nGx = 0, nGy = 0;\n'
           ...
           b'\t\t{\n'
           b'\t\t\t// BI CHAN DUNG GIUA DUONG (ro nhat luc qua cong thanh: server dat lai\n'
           b'\t\t\t// trang thai chien dau -> nhan vat ve do_stand). Duong tu tim van con\n'
           b'\t\t\t// nen khoi duoi VAN ban lenh buoc len server: server di tiep, client\n'
           b'\t\t\t// dung yen -> moi lan dong bo la mot cu TELE, va chinh cu tele do lai\n'
           b'\t\t\t// lam bo dem m_nAutoStall reset (coi nhu "co nhich") nen khong bao gio\n'
           b'\t\t\t// dung -> tele lien tuc toi tan dich.\n'
           b'\t\t\t// Cho nhan vat chay lai NGAY tai day (RunTo = cuc bo, khong gui goi),\n'
           ...
           b'\t\tint nKdx = nKx - m_DesX; if (nKdx < 0) nKdx = -nKdx;\n'
           b'\t\tint nKdy = nKy - m_DesY; if (nKdy < 0) nKdy = -nKdy;\n'
           b'\t\tif (nKdx <= 48 && nKdy <= 48 && m_nAutoPathIdx + 1 < m_nAutoPathCnt)\n'
           b'\t\t{\n'
           b'\t\t\t// DA dung han o waypoint (nga re) ma con waypoint -> di tiep NGAY,\n'
           b'\t\t\t// khong cho 45 khung (backup cho look-ahead o ServeMove).\n'
           b'\t\t\tm_nAutoPathIdx++;\n'
           b'\t\t\tm_DesX = m_AutoPathX[m_nAutoPathIdx]; m_DesY = m_AutoPathY[m_nAutoPathIdx];\n'
           b'\t\t\tm_nAutoPathNoProg = 0; m_nAutoPathLastDist = 0x7fffffff;\n'
           b'\t\t\textern void SendClientCmdRun(int nX, int nY);\n'
           ...
           b'\t\t{\n'
           b'\t\t\tm_nAutoPathNoProg = 0;\n'
           b'\t\t\t// m_nAutoFarX/Y LUON la dich that (GotoWhere set vo dieu kien), du dich\n'
           b'\t\t\t// gan (m_bAutoFar=0, A* toi-dich) hay xa. Kick toi no BAT KE m_bAutoFar:\n'
           b'\t\t\t// dich gan ma ket giua duong cung phai re-path vong qua, khong bo cuoc.\n'
           b'\t\t\tint nKfx = nKx - m_nAutoFarX; if (nKfx < 0) nKfx = -nKfx;\n'
           b'\t\t\tint nKfy = nKy - m_nAutoFarY; if (nKfy < 0) nKfy = -nKfy;\n'
           b'\t\t\tif (m_nAutoPathRecalc >= 40 || (nKfx <= 48 && nKfy <= 48))\n'
           b'\t\t\t{\n'
...
edit('S3Client/Ui/UiCase/UiSkills.cpp',
     ...
     _crlf(b'\t\t\tm_FightSkills[i].EnablePickPut(false);\n'
           ...
           b'\t\t\t\t/* O vuong 13x13 de len GOC DUOI PHAI icon chieu. Ban dau\n'
           ...
           b'\t\t\t\tm_ConDiemBtn[i].SetSize(13, 13);\n'
           b'\t\t\t\tm_ConDiemBtn[i].SetPosition(nL + nW - 13, nT + nH - 13);\n'
           b'\t\t\t}'),
...
edit('S3Client/Ui/UiCase/UiSkills.cpp',
     ...
     _crlf(b'/* O vuong DO vien DEN, dau "+" TRANG canh giua, ve o goc duoi phai\n'
           b'   icon chieu. Ke bang KRULine chu khong dung anh: khong phai them tai\n'
           b'   nguyen vao pak.\n'
           b'\n'
           b'   Tu ve chu chu khong goi KWndPureTextBtn::PaintWindow: ham do dat chu\n'
           b'   o DINH cua so (param.nY = m_nAbsoluteTop) va tinh be ngang theo\n'
           b'   nFontSize/2, nen dau + luon lech len tren va sang trai. Do tren anh\n'
           b'   chup 07/09/2026: glyph "+" font 12 rong 5px cao 7px va co san 1px dem\n'
           b'   phia tren -> lech 1px moi chieu trong o 13x13. */\n'
           b'void KNutCongDiem::PaintWindow()\n'
           b'{\n'
           b'\tif (!m_bCoChieu || g_pRepresentShell == NULL || m_Width <= 4 || m_Height <= 4)\n'
           b'\t\treturn;\n'
           b'\tKRULine To[40];\n'
           b'\tint nSo = 0;\n'
           b'\t/* Nen do: chua tu hang 2 de danh cho vien DAY 2px. */\n'
           b'\tfor (int nY = 2; nY < m_Height - 2 && nSo < 36; nY++, nSo++)\n'
           b'\t{\n'
           b'\t\tTo[nSo].Color.Color_dw = 0xffb02020;\n'
           b'\t\tTo[nSo].oPosition.nX = m_nAbsoluteLeft + 2;\n'
           b'\t\tTo[nSo].oEndPos.nX = m_nAbsoluteLeft + m_Width - 2;\n'
           b'\t\tTo[nSo].oPosition.nY = To[nSo].oEndPos.nY = m_nAbsoluteTop + nY;\n'
           b'\t}\n'
           b'\tif (nSo > 0)\n'
           b'\t\tg_pRepresentShell->DrawPrimitives(nSo, To, RU_T_LINE, true);\n'
           b'\t/* Vien den DAY 2px: hai khung long nhau. */\n'
           b'\tnSo = 0;\n'
           b'\tfor (int nV = 0; nV < 2; nV++)\n'
           b'\t{\n'
           b'\t\tint nL = m_nAbsoluteLeft + nV, nT = m_nAbsoluteTop + nV;\n'
           b'\t\tint nR = m_nAbsoluteLeft + m_Width - 1 - nV;\n'
           b'\t\tint nB = m_nAbsoluteTop + m_Height - 1 - nV;\n'
           b'\t\tTo[nSo].oPosition.nX = To[nSo].oEndPos.nX = nL;\n'
           b'\t\tTo[nSo].oPosition.nY = nT; To[nSo].oEndPos.nY = nB + 1; nSo++;\n'
           b'\t\tTo[nSo].oPosition.nX = To[nSo].oEndPos.nX = nR;\n'
           b'\t\tTo[nSo].oPosition.nY = nT; To[nSo].oEndPos.nY = nB + 1; nSo++;\n'
           b'\t\tTo[nSo].oPosition.nY = To[nSo].oEndPos.nY = nT;\n'
           b'\t\tTo[nSo].oPosition.nX = nL; To[nSo].oEndPos.nX = nR + 1; nSo++;\n'
           b'\t\tTo[nSo].oPosition.nY = To[nSo].oEndPos.nY = nB;\n'
           b'\t\tTo[nSo].oPosition.nX = nL; To[nSo].oEndPos.nX = nR + 1; nSo++;\n'
           b'\t}\n'
           b'\tfor (int nK = 0; nK < nSo; nK++)\n'
           b'\t\tTo[nK].Color.Color_dw = 0xff100808;\n'
           b'\tg_pRepresentShell->DrawPrimitives(nSo, To, RU_T_LINE, true);\n'
           b'\t/* Dau "+": lay dung be rong/cao THAT cua glyph (5x7 o font 12) roi\n'
           b'\t   canh giua, tru 1px dem san phia tren. */\n'
           b'\tg_pRepresentShell->OutputText(12, "+", 1,\n'
           b'\t\tm_nAbsoluteLeft + (m_Width - 5) / 2,\n'
           b'\t\tm_nAbsoluteTop + (m_Height - 7) / 2 - 1,\n'
           b'\t\t0xffffffff, 0xff000000);\n'
           b'}\n'
           b'\n'
           b'void KUiFightSkillSubPage::PaintWindow()'),
     've nut + do vien den, dau cong canh giua')
...
# DOI HANH VI: dan bay nhanh hon mot o moi nhip thi tu huy - chia nho buoc.
edit('Core/Src/KMissle.cpp',
     b'\tif (CheckBeyondRegion(nDOffsetX, nDOffsetY))\r\n\t{\r\n\t\tif (CheckCollision() == -1) \r\n\t\t{\r\n\t\t\tif (m_bAutoExplode)\r\n\t\t\t{\r\n\t\t\t\tProcessCollision();//\xb4\xa6\xc0\xed\xc5\xf6\xd7\xb2\r\n\t\t\t}\r\n#ifndef _SERVER \r\n\t\t\tint nSrcX4 = 0 ;\r\n\t\t\tint nSrcY4 = 0 ;\r\n\t\t\tSubWorld[0].Map2Mps(m_nRegionId, m_nCurrentMapX, m_nCurrentMapY,m_nXOffset, m_nYOffset, &nSrcX4, &nSrcY4);\r\n\t\t\tCreateSpecialEffect(MS_DoVanish, nSrcX4, nSrcY4, m_nCurrentMapZ);\r\n#endif\r\n\t\t\tDoVanish();\r\n\t\t\treturn;\r\n\t\t}\r\n\t}\r\n\telse//\xc8\xe7\xb9\xfb\xd7\xd3\xb5\xaf\xb7\xc9\xd0\xd0\xb9\xfd\xb3\xcc\xd6\xd0\xbd\xf8\xc8\xeb\xc1\xcb\xd2\xbb\xb8\xf6\xce\xde\xd0\xa7\xb5\xc4Region\xd4\xf2\xd7\xd3\xb5\xaf\xd7\xd4\xb6\xaf\xcf\xfb\xcd\xf6\r\n\t{\r\n\t\tDoVanish();\r\n\t}\r\n}\r\n',
...
edit('Core/Src/KMissle.cpp',
     ...
     'dan bam muc tieu: giu luat 8.x, doi /1.3 thanh *10/13 cho khop may chu')

edit('Core/Src/KMissle.cpp',
     b'\tcase\tMISSLE_MMK_Parabola:\t\t\t\t\t\t//\t\xc5\xd7\xce\xef\xcf\xdf\r\n\tcase\tMISSLE_MMK_Line:\t\t\t\t\t\t\t//\t\xd6\xb1\xcf\xdf\xb7\xc9\xd0\xd0\r\n\t\t{\t\r\n\t\t\tif(this->m_nFollowNpcIdx > 0 && this->m_eFollowKind == 2)\r\n\t\t\t{\r\n\t\t\t\t//\t\t\t\tm_nFollowNpcIdx = 2;\r\n\t\t\t\tif(m_nTempParam1 > 5)\r\n\t\t\t\t{\r\n\t\t\t\t\tint nDistance = 0;\r\n\t\t\t\t\tint nSrcMpsX = 0;\r\n\t\t\t\t\tint nSrcMpsY = 0;\r\n\t\t\t\t\tint nDesMpsX = 0;\r\n\t\t\t\t\tint nDesMpsY = 0;\r\n\t\t\t\t\t\r\n\t\t\t\t\tGetMpsPos(&nSrcMpsX,&nSrcMpsY);\r\n\t\t\t\t\tNpc[m_nFollowNpcIdx].GetMpsPos(&nDesMpsX,&nDesMpsY);\r\n\t\t\t\t\tnDistance = SubWorld[m_nSubWorldId].GetDistance(nSrcMpsX,nSrcMpsY,nDesMpsX,nDesMpsY);\r\n\t\t\t\t\tif (nDistance > 20)\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\tint nXFactor = ((nDesMpsX - nSrcMpsX) << 10)/nDistance;\r\n\t\t\t\t\t\tint nYFactor = ((nDesMpsY - nSrcMpsY) << 10)/nDistance;\r\n\t\t\t\t\t\tint dx = nXFactor * m_nSpeed / 1.3;\r\n\t\t\t\t\t\tint dy = nYFactor * m_nSpeed / 1.3;\r\n\t\t\t\t\t\tnDOffsetX = dx;\r\n\t\t\t\t\t\tnDOffsetY = dy;\r\n\t\t\t\t\t\tm_nDir = g_GetDirIndex(nSrcMpsX,nSrcMpsY,nDesMpsX,nDesMpsY);\r\n\t\t\t\t\t}\r\n\t\t\t\t\telse\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\tProcessDamage(m_nFollowNpcIdx);\r\n\t\t\t\t\t\tDoVanish();\r\n\t\t\t\t\t\treturn;\r\n\t\t\t\t\t}\r\n\t\t\t\t}else{\r\n\t\t\t\t\tnDOffsetX    = (m_nSpeed * m_nXFactor);\r\n\t\t\t\t\tnDOffsetY\t = (m_nSpeed * m_nYFactor);\r\n\t\t\t\t\tm_nTempParam1 ++;\r\n\t\t\t\t}\r\n\t\t\t}\r\n\t\t\telse\r\n\t\t\t{\r\n\t\t\t\tnDOffsetX    = (m_nSpeed * m_nXFactor);\r\n\t\t\t\tnDOffsetY\t = (m_nSpeed * m_nYFactor);\r\n\t\t\t}\r\n\t\t}\r\n\t\tbreak;\r\n',
     b'\tcase\tMISSLE_MMK_Parabola:\t\t\t\t\t\t//\t\xc5\xd7\xce\xef\xcf\xdf\r\n\tcase\tMISSLE_MMK_Line:\t\t\t\t\t\t\t//\t\xd6\xb1\xcf\xdf\xb7\xc9\xd0\xd0\r\n\t\t{\t\r\n\t\t\tif(this->m_nFollowNpcIdx > 0 && this->m_eFollowKind == 2)\r\n\t\t\t{\r\n\t\t\t\t//\t\t\t\tm_nFollowNpcIdx = 2;\r\n\t\t\t\tif(m_nTempParam1 > 5)\r\n\t\t\t\t{\r\n\t\t\t\t\tint nDistance = 0;\r\n\t\t\t\t\tint nSrcMpsX = 0;\r\n\t\t\t\t\tint nSrcMpsY = 0;\r\n\t\t\t\t\tint nDesMpsX = 0;\r\n\t\t\t\t\tint nDesMpsY = 0;\r\n\t\t\t\t\t\r\n\t\t\t\t\tGetMpsPos(&nSrcMpsX,&nSrcMpsY);\r\n\t\t\t\t\tNpc[m_nFollowNpcIdx].GetMpsPos(&nDesMpsX,&nDesMpsY);\r\n\t\t\t\t\tnDistance = SubWorld[m_nSubWorldId].GetDistance(nSrcMpsX,nSrcMpsY,nDesMpsX,nDesMpsY);\r\n\t\t\t\t\tif (nDistance > 20)\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\tint nXFactor = ((nDesMpsX - nSrcMpsX) << 10)/nDistance;\r\n\t\t\t\t\t\tint nYFactor = ((nDesMpsY - nSrcMpsY) << 10)/nDistance;\r\n\t\t\t\t\t\t/* *10/13 thay cho /1.3, cho khop so nguyen ben may chu. */\r\n\t\t\t\t\t\tint dx = nXFactor * m_nSpeed * 10 / 13;\r\n\t\t\t\t\t\tint dy = nYFactor * m_nSpeed * 10 / 13;\r\n\t\t\t\t\t\tnDOffsetX = dx;\r\n\t\t\t\t\t\tnDOffsetY = dy;\r\n\t\t\t\t\t\t/* Ghi lai he so huong: khi muc tieu chet, nhanh duoi bay\r\n\t\t\t\t\t\t   tiep bang hai he so nay. De nguyen he so LUC BAN thi dan\r\n\t\t\t\t\t\t   re nguoc ve huong cu trong khi hinh van quay theo m_nDir\r\n\t\t\t\t\t\t   moi - nhin ra ngoai la con rong nam ngang. */\r\n\t\t\t\t\t\tm_nXFactor = nXFactor;\r\n\t\t\t\t\t\tm_nYFactor = nYFactor;\r\n\t\t\t\t\t\tm_nDir = g_GetDirIndex(nSrcMpsX,nSrcMpsY,nDesMpsX,nDesMpsY);\r\n\t\t\t\t\t}\r\n\t\t\t\t\telse\r\n\t\t\t\t\t{\r\n\t\t\t\t\t\tProcessDamage(m_nFollowNpcIdx);\r\n\t\t\t\t\t\tDoVanish();\r\n\t\t\t\t\t\treturn;\r\n\t\t\t\t\t}\r\n\t\t\t\t}else{\r\n\t\t\t\t\tnDOffsetX    = (m_nSpeed * m_nXFactor);\r\n\t\t\t\t\tnDOffsetY\t = (m_nSpeed * m_nYFactor);\r\n\t\t\t\t\tm_nTempParam1 ++;\r\n\t\t\t\t}\r\n\t\t\t}\r\n\t\t\telse\r\n\t\t\t{\r\n\t\t\t\tnDOffsetX    = (m_nSpeed * m_nXFactor);\r\n\t\t\t\tnDOffsetY\t = (m_nSpeed * m_nYFactor);\r\n\t\t\t}\r\n\t\t}\r\n\t\tbreak;\r\n',
     'nhanh bay thang: giu khoi bam muc tieu, doi /1.3 thanh *10/13')
...
# DOI HANH VI: dan bam muc tieu cua chieu hinh TUONG phai xuat phat tu nguoi.
edit('Core/Src/KSkills.cpp',
     b'\t\t\t\t\tCastWall(&SkillParam , nDir, nDesPX, nDesPY);\r\n',
