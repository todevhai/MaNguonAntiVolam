...
edit('S3Client/Ui/UiCase/UiToolsControlBar.cpp',
     ...
     _crlf(b'\tcase WND_M_MENUITEM_SELECTED:\n'
           ...
           b'\t\t\t{\n'
           b'\t\t\t\t/* Do sat di duong RIENG: c2s_pkapplyenmity, va bat buoc co\n'
           b'\t\t\t\t   nguoi bi nham. Khong co muc tieu thi may chu bo qua. */\n'
           b'\t\t\t\tKUiPlayerItem sMucTieu;\n'
           b'\t\t\t\tmemset(&sMucTieu, 0, sizeof(sMucTieu));\n'
           b'\t\t\t\tif (g_pCoreShell->FindSelectNPC(KShortcutKeyCentre::ms_MouseX,\n'
           b'\t\t\t\t\tKShortcutKeyCentre::ms_MouseY, relation_enemy, false,\n'
           b'\t\t\t\t\t&sMucTieu, nKindTam))\n'
           b'\t\t\t\t\tg_pCoreShell->OperationRequest(GOI_REVENGE_SOMEONE,\n'
           b'\t\t\t\t\t\t(unsigned int)&sMucTieu, 0);\n'
           b'\t\t\t}\n'
           b'\t\t\tKPopupMenu::Cancel();\n'
           b'\t\t}\n'
           b'\t\tbreak;\n'
