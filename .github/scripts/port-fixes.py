...
edit('Core/Src/KNpc.cpp',
     ...
     _crlf(b'\tBOOL bApStuck = FALSE;\n'
           ...
           b'\t\t}\n'
           b'\t}\n'
           b'\t// LOOK-AHEAD: gan toi waypoint (VAN dang di) ma con waypoint ke -> nhay\n'
           b'\t// som + gui waypoint ke NGAY, de server KHONG dung o nga re. Truoc day\n'
           b'\t// chi nhay waypoint khi da DUNG han (nRet==0) -> server dung o moi goc,\n'
           b'\t// phai cho kick 45 khung moi di tiep -> "dung khung o nga re".\n'
           b'\tif (IsPlayer() && m_nAutoPathCnt > 0 && nRet == 1 && !bApStuck\n'
           b'\t\t&& m_nAutoPathIdx + 1 < m_nAutoPathCnt)\n'
           b'\t{\n'
           b'\t\tint nLaMx = x >> 10, nLaMy = y >> 10;\n'
           b'\t\tint nLax = nLaMx - m_DesX; if (nLax < 0) nLax = -nLax;\n'
           b'\t\tint nLay = nLaMy - m_DesY; if (nLay < 0) nLay = -nLay;\n'
           b'\t\tif (nLax <= 80 && nLay <= 80)\n'
           b'\t\t{\n'
           b'\t\t\tm_nAutoPathIdx++;\n'
           b'\t\t\tm_DesX = m_AutoPathX[m_nAutoPathIdx];\n'
           b'\t\t\tm_DesY = m_AutoPathY[m_nAutoPathIdx];\n'
           ...
           b'\t\t\textern void SendClientCmdRun(int nX, int nY);\n'
           b'\t\t\tif (m_Doing == do_run) SendClientCmdRun(m_DesX, m_DesY); else SendClientCmdWalk(m_DesX, m_DesY);\n'
           b'\t\t\tg_DebugLog("[AUTOPATH] lookahead %d/%d des=%d,%d", m_nAutoPathIdx, m_nAutoPathCnt, m_DesX, m_DesY);\n'
           b'\t\t}\n'
           b'\t}\n'
