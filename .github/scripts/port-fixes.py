...
edit('Engine/Src/KDrawBase.cpp',
     ...
     b'		static int s_nKieuTron = 0;\t/* 0 screen, 1 cong, 2 alpha */\r\n'
     b'		if (s_nCuongDo < 0)\r\n'
     b'		{\r\n'
     ...
     b'				   bao gio cham tran. */\r\n'
     b'				int nSR = ((((wNguon >> nDichR) & nTranR) * nHeSo) >> 5);\r\n'
     b'				int nSG = ((((wNguon >> nDichG) & nTranG) * nHeSo) >> 5);\r\n'
     b'				int nSB = (((wNguon & nTranB) * nHeSo) >> 5);\r\n'
     ...
     b'				{\r\n'
     b'					nR = nDR + (((nSR - nDR) * nHeSo) >> 5);\r\n'
     b'					nG = nDG + (((nSG - nDG) * nHeSo) >> 5);\r\n'
     b'					nB = nDB + (((nSB - nDB) * nHeSo) >> 5);\r\n'
     b'					if (nR < 0) nR = 0;\r\n'
     b'					if (nG < 0) nG = 0;\r\n'
     b'					if (nB < 0) nB = 0;\r\n'
     b'				}\r\n'
...
# DOI HANH VI: nhanh dan BAM MUC TIEU va nhanh bay THANG cua client chay luat
# khac may chu -> duong bay nhin ky. Dua ve dung luat ban 2003.
edit('Core/Src/KMissle.cpp',
     ...
     b'\tcase\tMISSLE_MMK_Follow:\t\t\t\t\t\t\t//\t\xb8\xfa\xd7\xd9\xc4\xbf\xb1\xea\xb7\xc9\xd0\xd0\r\n\t\t{\r\n\t\t\t/* DOI HANH VI: cho khop LUAT CUA MAY CHU (Core/KMissle.cpp ban 2003).\r\n\t\t\t   Ban 8.x o day them ba thu ma may chu khong co: nam nhip dau bay\r\n\t\t\t   THANG theo huong cu, roi moi be sang muc tieu; toc do khi duoi bi\r\n\t\t\t   chia 1.3; va tu no gay sat thuong roi bien mat khi con cach 20.\r\n\t\t\t   Hai nua chay hai luat khac nhau nen duong bay nhin "ky": dan giat\r\n\t\t\t   mot cai roi di cham hon cho may chu tinh. */\r\n\t\t\tif (this->m_nFollowNpcIdx > 0)\r\n\t\t\t{\r\n\t\t\t\tint nDistance = 0;\r\n\t\t\t\tint nSrcMpsX = 0;\r\n\t\t\t\tint nSrcMpsY = 0;\r\n\t\t\t\tint nDesMpsX = 0;\r\n\t\t\t\tint nDesMpsY = 0;\r\n\r\n\t\t\t\tGetMpsPos(&nSrcMpsX, &nSrcMpsY);\r\n\t\t\t\tNpc[m_nFollowNpcIdx].GetMpsPos(&nDesMpsX, &nDesMpsY);\r\n\t\t\t\tnDistance = SubWorld[m_nSubWorldId].GetDistance(nSrcMpsX, nSrcMpsY, nDesMpsX, nDesMpsY);\r\n\t\t\t\tif (nDistance != 0)\r\n\t\t\t\t{\r\n\t\t\t\t\tint nXFactor = ((nDesMpsX - nSrcMpsX) << 10) / nDistance;\r\n\t\t\t\t\tint nYFactor = ((nDesMpsY - nSrcMpsY) << 10) / nDistance;\r\n\t\t\t\t\tnDOffsetX = nXFactor * m_nSpeed;\r\n\t\t\t\t\tnDOffsetY = nYFactor * m_nSpeed;\r\n\t\t\t\t\tm_nDir = g_GetDirIndex(nSrcMpsX, nSrcMpsY, nDesMpsX, nDesMpsY);\r\n\t\t\t\t}\r\n\t\t\t\telse\r\n\t\t\t\t{\r\n\t\t\t\t\t/* Da nam dung o muc tieu: bay tiep theo huong cu thay vi dung im. */\r\n\t\t\t\t\tnDOffsetX = (m_nSpeed * m_nXFactor);\r\n\t\t\t\t\tnDOffsetY = (m_nSpeed * m_nYFactor);\r\n\t\t\t\t}\r\n\t\t\t}\r\n\t\t\telse\r\n\t\t\t{\r\n\t\t\t\tnDOffsetX    = (m_nSpeed * m_nXFactor);\r\n\t\t\t\tnDOffsetY\t = (m_nSpeed * m_nYFactor);\r\n\t\t\t}\r\n\t\t}break;\r\n',
     'dan bam muc tieu: bo tre 5 nhip, bo chia 1.3, bo nguong 20 - khop may chu')
